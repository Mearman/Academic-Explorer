...
export function openalexCachePlugin(options: OpenAlexCachePluginOptions = {}): Plugin {
  ...
  /**
   * Update directory indexes after caching new file
   */
  const updateDirectoryIndexes = async (cachePath: string, url: string, fileName: string, retrieved_at?: string, contentHash?: string): Promise<void> => {
    try {
      ...
      await updateDirectoryIndex(dir, url, fileName, retrieved_at, contentHash);

      // Also update root index if this is a top-level entity directory
      const relativePath = dir.replace(staticDataDir, '').replace(/^\//, '');
      const pathSegments = relativePath.split('/').filter(Boolean);

      if (pathSegments.length === 1) {
        // This is a top-level entity directory (e.g., "authors", "works")
  ...
  /**
   * Update a single directory index file
   */
  const updateDirectoryIndex = async (dirPath: string, triggerUrl: string, newFileName?: string, retrieved_at?: string, contentHash?: string): Promise<void> => {
    try {
      ...
      logVerbose(`Updated directory index: ${index.path}`);
