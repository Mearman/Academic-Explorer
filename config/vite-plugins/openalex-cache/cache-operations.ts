...
// NOTE: DiskCacheWriter is NOT used in Vite plugin anymore to avoid workspace dependencies
// during config bundling in CI. Cache writing is disabled in dev mode.
// Production builds use the static cache generated by the CLI tool.
...
export async function saveToCache(
  ...
  try {
    ...
    // Cache writing is disabled in Vite plugin to avoid workspace dependencies
    // Production builds use the static cache generated by the CLI tool
    logVerbose(`[DEV] Cache writing disabled in Vite plugin - use CLI tool to generate static cache`);
  } catch (error) {
    const logVerbose = createLogVerbose(context.verbose);
