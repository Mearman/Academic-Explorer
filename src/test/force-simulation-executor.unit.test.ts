...
import { createForceSimulationExecutor } from "@/lib/graph/execution/force-simulation-executor";
import type { ForceSimulationTask } from "@/lib/graph/execution";
import type {
  ForceSimulationNode,
  ForceSimulationLink
...
describe("ForceSimulationExecutor", () => {
  ...
  it("should handle FORCE_SIMULATION_UPDATE_NODES task", async () => {
    ...
    const updatedNodes = [
      ...createTestNodes(),
      { id: "node4", type: "topics", x: 200, y: 200 }
    ];
  ...
  it("should reject invalid task types", async () => {
    ...
    await expect(executor(invalidTask, emitSpy)).rejects.toThrow(
      "Unknown force simulation task type: INVALID_TASK_TYPE"
    );
  });
