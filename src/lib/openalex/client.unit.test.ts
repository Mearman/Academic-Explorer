...
describe('OpenAlexBaseClient', () => {
  ...
  beforeEach(() => {
    ...
    const createMockResponse = (data = { results: [], meta: { count: 0, db_response_time_ms: 10, page: 1, per_page: 25 } }) =>
      new Response(
        JSON.stringify(data),
        {
          status: 200,
  ...
  describe('URL Building and Query Parameters', () => {
    ...
    it('should handle various query parameter types', async () => {
      ...
      expect(url.searchParams.get('select')).toBe('id,display_name,publication_year');
    ...
    it('should properly encode special characters in parameters', async () => {
      ...
      expect(url.searchParams.get('filter')).toBe('title.search:"deep learning"');
    });
  });
  ...
  describe('Error Handling', () => {
    it('should handle HTTP 404 errors without retries', async () => {
      ...
      await expect(client.get('works/W999999999')).rejects.toThrow(OpenAlexApiError);
      ...
      await expect(client.get('works/W999999999')).rejects.toThrow('Work not found');
    });
    ...
    it('should handle timeout errors', async () => {
      ...
      // Reset mock for second test
      mockFetch.mockRejectedValueOnce(timeoutError);
      await expect(client.get('works')).rejects.toThrow(/Request timeout after/);
    });

    it('should parse error response JSON when available', async () => {
      const errorResponse = new Response(
        JSON.stringify({ error: 'Bad Request', message: 'Invalid filter parameter' }),
      ...
      try {
        ...
        expect((error as OpenAlexApiError).message).toBe('Invalid filter parameter');
    ...
    it('should handle malformed error response JSON', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(OpenAlexApiError);
        expect((error as OpenAlexApiError).message).toBe('HTTP 400 Bad Request');
        expect((error as OpenAlexApiError).statusCode).toBe(400);
      }
    });
  });
  ...
  describe('Response Parsing', () => {
    it('should parse JSON responses correctly', async () => {
      const testData = { results: [{ id: 'W123', display_name: 'Test Work' }], meta: { count: 1 } };
  ...
  describe('HTTP Methods', () => {
    ...
    it('should make GET requests with getById method', async () => {
      ...
      const result = await client.getById('works', 'W123', { select: ['id', 'display_name'] });
  ...
  describe('Streaming and Pagination', () => {
    it('should stream results using async generator', async () => {
      ...
      for await (const batch of client.stream('works', {}, 2)) {
        results.push(batch);
      }
  ...
  describe('Edge Cases and Error Conditions', () => {
    ...
    it('should preserve original error types in network failures', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(OpenAlexApiError);
        expect((error as OpenAlexApiError).message).toContain('Failed to fetch');
      }
    });
  });
});