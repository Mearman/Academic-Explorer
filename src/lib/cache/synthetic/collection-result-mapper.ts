...
export class CollectionResultMapper implements StorageTierInterface {
  ...
  async putCollectionPage(queryKey: string, page: number, entityIds: string[]): Promise<void> {
    ...
    if (Math.random() < 0.05) { // 5% chance
      void this.cleanupExpiredCollections();
    }
  }
   ...
   */
  async getCollectionMetadata(queryKey: string): Promise<CollectionMetadata | null> {
    const collection = this.memoryCollections.get(queryKey);
   ...
   */
  async putCollectionMetadata(queryKey: string, metadata: CollectionMetadata): Promise<void> {
    let collection = this.memoryCollections.get(queryKey);
   ...
   */
  async deleteCollection(queryKey: string): Promise<void> {
  ...
  getCachedEntityCount(queryKey: string): number {
    ...
    return Object.values(collection.pages)
      .reduce((total, entityIds) => total + entityIds.length, 0);
  }
   ...
   */
  async markCollectionComplete(queryKey: string, totalCount: number): Promise<void> {
    const collection = this.memoryCollections.get(queryKey);
    if (collection) {
  ...
  async getEntityFields(): Promise<Partial<any>> {
    return {}; // CollectionResultMapper doesn't handle entity fields
  }
  ...
  async getFieldCoverage(): Promise<string[]> {
    return []; // CollectionResultMapper doesn't handle entity fields
  }

  async hasFields(): Promise<boolean> {
    return false; // CollectionResultMapper doesn't handle entity fields
  }
  ...
  private getCollectionTTL(queryKey: string): number {
    ...
    const entityType = queryKey.split('|')[0] as EntityType;
    return this.policy.collectionTTL[entityType] || 24 * 60 * 60 * 1000; // Default 24 hours
  }
  ...
  private extractSortFromQueryKey(queryKey: string): string | undefined {
    ...
    return sortPart ? JSON.parse(sortPart.replace('sort:', '')) : undefined;
  }
  ...
  private async cleanupExpiredCollections(): Promise<void> {
  ...
  private estimateCollectionSize(collection: CollectionResultMapping[string]): number {
    try {
      ...
      const avgEntitiesPerPage = Object.values(collection.pages)
        .reduce((sum, entities) => sum + entities.length, 0) / Math.max(pageCount, 1);
