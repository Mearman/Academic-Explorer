...
export interface SyntheticCacheLayer {
  ...
  getCollectionMetadata(queryKey: string): Promise<any>;
...
export class SyntheticCacheLayerImpl implements SyntheticCacheLayer {
   ...
   */
  async getCollectionMetadata(queryKey: string): Promise<any> {
    try {
  ...
  async analyzeMultiTierAvailability(
    ...
    try {
      ...
      const entityType = 'works' as EntityType; // Would need to be passed in or inferred
  ...
  async getStorageTierStats(): Promise<MultiTierCacheStats> {
    try {
      ...
      const fieldAccumulatorStats = await this.fieldAccumulator.getStats();
      ...
      return {
        overall: {
          ...
          totalCollections: collectionMapperStats[StorageTier.MEMORY]?.collections || 0,
