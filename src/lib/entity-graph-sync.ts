...
export async function loadEntityGraphFromSimpleStorage(): Promise<{
  ...
  const simpleGraph = await entityGraphStorage.load();
...
export async function saveEntityToSimpleStorage(
  ...
  // Upsert the entity
  await entityGraphStorage.upsertEntity(entityId, entityType, displayName);
  
  // Mark as visited if this is a direct visit
  if (isVisited) {
    await entityGraphStorage.markEntityVisited(entityId);
  }
}
...
export async function saveEdgeToSimpleStorage(
  ...
  // Initialize IndexedDB first
  await entityGraphStorage.init();
  await entityGraphStorage.addEdge(sourceId, targetId, edgeType, edgeId);
}
