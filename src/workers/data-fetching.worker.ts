...
export interface DataFetchingMessage {
  ...
  payload?: any; // Optional for cancel messages
}
...
// Worker state
let isReady = false;
let currentRequests = new Map<string, AbortController>();
...
async function handleBatchExpand(id: string, payload: any) {
...
async function handleSearch(id: string, payload: any) {
...
self.onmessage = async (event: MessageEvent<DataFetchingMessage>) => {
  ...
  try {
    switch (type) {
      ...
      case 'batchExpand':
        await handleBatchExpand(id, payload);
        break;
