...
import type { ValidationSeverity, ValidationStatistics, ValidationIssueType } from '@/types/entity-validation';
...
// Component for summary statistics cards
function SummaryCards({ summary, statistics }: { summary: ValidationSummary; statistics: ValidationStatistics }) {
  const successRate = summary.totalEntitiesValidated > 0 
...
// Component for issue severity breakdown
function IssueSeverityBreakdown({ summary }: { summary: ValidationSummary }) {
  if (summary.totalIssues === 0) return null;

  const severityItems = [
    { key: 'error', label: 'Errors', icon: IconX, color: 'red' },
    { key: 'warning', label: 'Warnings', icon: IconAlertTriangle, color: 'orange' },
    { key: 'info', label: 'Info', icon: IconInfoCircle, color: 'blue' },
  ] as const;
  ...
  return (
    <Paper p="md" withBorder>
      ...
      <Stack gap="md">
        {severityItems.map(({ key, label, icon: Icon, color }) => {
          const count = summary.issuesBySeverity[key] || 0;
...
// Component for common issue types
function CommonIssueTypes({ statistics }: { statistics: ValidationStatistics }) {
  if (statistics.commonIssueTypes.length === 0) return null;
...
// Component for recent activity table
function RecentActivityTable({ statistics }: { statistics: ValidationStatistics }) {
  if (statistics.recentActivity.length === 0) return null;
...
export function ValidationDashboard() {
  ...
  return (
    <Stack gap="xl">
      ...
      <SummaryCards summary={summary} statistics={statistics} />
      <IssueSeverityBreakdown summary={summary} />
      <CommonIssueTypes statistics={statistics} />
      <RecentActivityTable statistics={statistics} />
    </Stack>
  );
}