...
interface FilterConfig {
  numeric?: Array<{ key: string; label: string; min: number; max?: number }>;
  select?: Array<{ key: string; label: string; options: Array<{ value: string; label: string }> }>;
  text?: Array<{ key: string; label: string }>;
  boolean?: Array<{ key: string; label: string }>;
  multiSelect?: Array<{ key: string; label: string; options: Array<{ value: string; label: string }> }>;
}
...
function getAuthorFilters(): FilterConfig {
  return {
    numeric: [
...
function getInstitutionFilters(): FilterConfig {
  return {
    numeric: [
      { key: 'cited_by_count', label: 'Citations', min: 0 },
      { key: 'works_count', label: 'Works', min: 0 },
...
function getSourceFilters(): FilterConfig {
  return {
    numeric: [
      { key: 'cited_by_count', label: 'Citations', min: 0 },
      { key: 'works_count', label: 'Works', min: 0 },
...
function getWorkFilters(): FilterConfig {
  return {
    numeric: [
      { key: 'cited_by_count', label: 'Citations', min: 0 },
...
function getOtherEntityFilters(entityType: EntityType): FilterConfig {
  switch (entityType) {
    case 'topic':
      return {
        numeric: [
...
function NumericFilters({ 
  ...
  if (!filterConfig.numeric?.length) return null;
...
function SelectFilters({ 
  ...
  if (!filterConfig.select?.length) return null;
...
function OtherFilterComponents({ 
  filters, 
  updateFilter, 
  filterConfig 
}: { 
...
export function EntityFilters({ entityType, filters, onChange }: EntityFiltersProps) {
  ...
  const filterConfig = useMemo(() => {
    switch (entityType) {
      case 'author': return getAuthorFilters();
      case 'institution': return getInstitutionFilters();
      case 'source': return getSourceFilters();
      case 'work': return getWorkFilters();
      default: return getOtherEntityFilters(entityType);
    }
  }, [entityType]);

  const hasAnyFilters = filterConfig.numeric?.length || filterConfig.select?.length || 
    filterConfig.text?.length || filterConfig.boolean?.length || filterConfig.multiSelect?.length;
  ...
  return (
    <Paper p="md" withBorder className={styles.filterSection}>
      ...
      <Stack gap="md">
        <NumericFilters filters={filters} updateFilter={updateFilter} filterConfig={filterConfig} />
        <SelectFilters filters={filters} updateFilter={updateFilter} filterConfig={filterConfig} />
        <OtherFilterComponents filters={filters} updateFilter={updateFilter} filterConfig={filterConfig} />
      </Stack>
    </Paper>
  );
}