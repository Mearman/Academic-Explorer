...
export function ValidationIndicator({
  ...
  // Don't show if validation is disabled
  if (!validationSettings.enabled || !validationSettings.showValidationIndicators) {
    return null;
  }

  // Don't show if this entity type is not being validated
  if (!validationSettings.validatedEntityTypes.includes(entityType)) {
    return null;
  }

  // Don't show if no validation has been performed
  if (!validationResult) {
    return null;
  }
  ...
  const getStatusIcon = () => {
    if (!hasIssues) {
      return <IconCheck size={size === 'xs' ? 12 : size === 'sm' ? 14 : 16} />;
    }

    switch (highestSeverity) {
      case 'error':
        return <IconX size={size === 'xs' ? 12 : size === 'sm' ? 14 : 16} />;
      case 'warning':
        return <IconAlertTriangle size={size === 'xs' ? 12 : size === 'sm' ? 14 : 16} />;
      case 'info':
        return <IconInfoCircle size={size === 'xs' ? 12 : size === 'sm' ? 14 : 16} />;
      default:
        return <IconCheck size={size === 'xs' ? 12 : size === 'sm' ? 14 : 16} />;
    }
  };
