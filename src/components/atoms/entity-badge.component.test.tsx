...
vi.mock('./badge', () => ({
  Badge: React.forwardRef<HTMLSpanElement, any>(({ children, className, size, 'data-testid': testId, 'aria-label': ariaLabel, ...props }, ref) => (
    <span 
      ref={ref}
      className={className}
      data-testid={testId}
...
vi.mock('./icon', () => ({
  Icon: ({ name, size, 'aria-hidden': ariaHidden, ...props }: any) => (
    <span 
...
describe('EntityBadge Component', () => {
  ...
  describe('Error Handling', () => {
    it('should handle invalid entityType gracefully', () => {
      expect(() => {
        render(<EntityBadge entityType={'invalid' as any} data-testid="entity-badge" />);
      }).not.toThrow();
    });

    it('should handle invalid size gracefully', () => {
      expect(() => {
        render(<EntityBadge entityType="author" size={'invalid' as any} />);
      }).not.toThrow();
    });
