...
function getEntityTypeFromPath(catchAll: string[]): { entityType: EntityType; entityId: string } | null {
  if (!catchAll || catchAll.length === 0) return null;
  
  // Join all segments back to form the original path
  const fullPath = catchAll.join('/');
  
  try {
    // Try to parse as entity identifier
    const result = parseEntityIdentifier(fullPath);
...
function renderEntityData(entityType: EntityType) {
  const EntityRenderer = (data: OpenAlexEntity) => {
    // This is a generic renderer - in a full implementation,
    // you'd want type-specific renderers
    return (
...
export default function CatchAllClient() {
  ...
  const catchAll = params?.id as string[] | undefined;

  // Attempt to detect entity type from the path
  const entityInfo = catchAll ? getEntityTypeFromPath(catchAll) : null;

  useEffect(() => {
    // Log for debugging in development
    if (process.env.NODE_ENV === 'development') {
      console.log('CatchAll route params:', { catchAll, entityInfo });
    }
  }, [catchAll, entityInfo]);
