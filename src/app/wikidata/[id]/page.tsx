...
export default function WikidataPage() {
  ...
  const wikidataId = params?.id as string;
  ...
  }, [wikidataId, router, searchParams]);

  // Show appropriate UI based on state
  if (state.status === 'error') {
    return (
      <EntityErrorBoundary entityType="wikidata">
        <WikidataError wikidataId={state.wikidata || wikidataId} error={state.error || 'Unknown error'} />
      </EntityErrorBoundary>
    );
  }
  ...
  return (
    <EntityErrorBoundary entityType="wikidata">
      <WikidataRedirectLoading wikidataId={state.wikidata || wikidataId} />
    </EntityErrorBoundary>
  );
}
