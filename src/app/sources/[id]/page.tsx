...
import type { SourcePageProps } from '@/app/types/page-params';
...
async function fetchSource(id: string): Promise<Source> {
  try {
    const normalisedId = normaliseEntityId(id, EntityType.SOURCE);
    const response = await openAlex.source(normalisedId);
    
    if (!response) {
      throw new Error(`Source not found: ${normalisedId}`);
    }
...
export async function generateStaticParams() {
  // Return empty array since we can't pre-generate all possible source IDs
...
export async function generateMetadata({
  ...
  try {
    ...
    if (!validateEntityId(id, EntityType.SOURCE) && !/^\d+$/.test(id)) {
      return {
        title: 'Source Not Found - Academic Explorer',
        description: 'The requested academic source could not be found.',
      };
    }

    const source = await fetchSource(id);
    const name = source.display_name || `Source ${source.id}`;
    
    const sourceType = source.type ? ` (${source.type})` : '';
    const publisher = source.host_organization_name ? ` by ${source.host_organization_name}` : '';
    
    const description = `Academic ${source.type || 'source'}: ${name}${publisher}. ${source.works_count.toLocaleString()} works, ${source.cited_by_count.toLocaleString()} citations. ${source.is_oa ? 'Open Access' : 'Subscription'}.`;
    
    return {
      title: `${name}${sourceType} - Academic Explorer`,
    ...
    return {
      title: 'Source - Academic Explorer',
      description: 'View academic source details and publications',
    };
  }
}
...
export default async function SourcesPage({ params }: SourcePageProps) {
  const { id } = await params;
  ...
  try {
    ...
    console.error('Error in sources page:', error);
    notFound();
  }
}
