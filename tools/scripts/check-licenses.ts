...
function checkLicenses(): void {
	try {
		...
		console.log(`[CLIPBOARD] Configuration loaded: ${config.forbidden.length} forbidden licenses`)
		...
		for (const [packageName, pkg] of Object.entries(licenseData)) {
			if (pkg.licenses && config.forbidden.some((forbidden) => pkg.licenses.includes(forbidden))) {
				violations.push({
		...
		if (violations.length > 0) {
			...
			console.log(`[SUCCESS] All ${totalPackages} packages have acceptable licenses`)
		...
		if (nodeError.code === "ETIMEDOUT") {
			...
			console.warn("[WARNING] License check was terminated - likely due to system resource limits")
			console.log(LICENSE_COMPLIANCE_MESSAGE)
