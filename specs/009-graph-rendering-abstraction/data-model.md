...
**State Transitions**:
1. **Created** � Initialized with id, type, position (x, y)
2. **Simulating** � Velocity (vx, vy) updated by forces; position updated by integrator
3. **Fixed** � Position locked, velocity zeroed, forces ignored
4. **Removed** � Deleted from graph; orphan edges become invalid
...
**State Transitions**:
1. **Created** � Initialized with id, type, source, target node references
2. **Active** � Participates in force calculations and rendering
3. **Hidden** � Participates in forces but excluded from rendering
4. **Orphaned** � Source or target node removed; edge becomes invalid
5. **Removed** � Deleted from graph
...
| Field | Type | Required | Description | Validation |
|-------|------|----------|-------------|------------|
| `nodes` | `Map<string, TNode>` | Yes | Node storage (id � node) | Unique IDs |
| `edges` | `Map<string, TEdge>` | Yes | Edge storage (id � edge) | Unique IDs; valid references |
...
| `visualConfig` | `VisualConfigMap<TNode, TEdge>` | Type � visual property mapping |
...
interface VisualConfigMap<TNode extends Node, TEdge extends Edge> {
  node: Record<string, NodeVisualConfig>; // node.type � config
  edge: Record<string, EdgeVisualConfig>; // edge.type � config
}
```
