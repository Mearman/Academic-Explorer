...
This task list implements automated PostHog source map upload for production error tracking in BibGraph. Tasks are organized by user story priority (P1 → P2 → P3) to enable independent, incremental implementation and testing.
...
**User Story**: When a production error occurs in BibGraph, developers need to see the original source code context (file names, line numbers, function names) rather than minified/bundled code references.
