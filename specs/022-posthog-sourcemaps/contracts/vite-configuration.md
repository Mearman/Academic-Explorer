...
This document defines the Vite configuration changes required to enable source map generation for PostHog error tracking. Vite is the build tool for BibGraph's web application.
...
**File Count**: One `.map` file per `.js` bundle/chunk (expected 10-50 for BibGraph)
