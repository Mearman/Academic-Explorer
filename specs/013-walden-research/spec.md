...
As a researcher using BibGraph, I need to access the improved OpenAlex Data Version 2 (Walden) so that I can benefit from better citation parsing, location detection, and language identification.
...
**Independent Test**: Can be fully tested by querying works via BibGraph and verifying the response includes improved metadata (more references, more locations, better language detection) compared to Data Version 1.

**Acceptance Scenarios**:

1. **Given** BibGraph is running, **When** a user searches for works, **Then** the system automatically uses OpenAlex Data Version 2 by default
...
- What happens when xpac works have missing or incomplete metadata that BibGraph normally expects?
...
- BibGraph defaults `include_xpac=true` to align with OpenAlex's future direction (they plan to make xpac default but timing is TBD)
- xpac works will eventually receive full author disambiguation, but no timeline specified
- Fulltext indexing issues in Walden will be resolved by OpenAlex team (not BibGraph responsibility)
...
The following improvements are documented in the Walden release and should be reflected in BibGraph:
