...
export const EntityCardGrid: React.FC<EntityCardGridProps> = ({
	...
	const renderCard = (entity: OpenAlexEntity, index: number) => {
		...
		if (isWork(entity)) {
			return <WorkCard key={key} work={entity as Work} onNavigate={onNavigate} />
		}

		if (isAuthor(entity)) {
			return <AuthorCard key={key} author={entity as Author} onNavigate={onNavigate} />
		}

		if (isSource(entity)) {
			return <SourceCard key={key} source={entity as Source} onNavigate={onNavigate} />
		}

		if (isInstitution(entity)) {
			return (
				<InstitutionCard key={key} institution={entity as InstitutionEntity} onNavigate={onNavigate} />
			)
		}

		if (isTopic(entity)) {
			return <TopicCard key={key} topic={entity as Topic} onNavigate={onNavigate} />
		}

		if (isPublisher(entity)) {
			return <PublisherCard key={key} publisher={entity as Publisher} onNavigate={onNavigate} />
		}

		if (isFunder(entity)) {
			return <FunderCard key={key} funder={entity as Funder} onNavigate={onNavigate} />
		}
		...
		if (isInstitution(entity)) {
			return <InstitutionCard key={key} institution={entity} onNavigate={onNavigate} />
		}
		...
		if (isPublisher(entity)) {
			return <PublisherCard key={key} publisher={entity} onNavigate={onNavigate} />
		}
		...
		return (
			<EntityCard
				...
				displayName={
					(entityRecord.display_name as string) || (entityRecord.title as string) || "Unknown Entity"
				}
...
function getEntityTypeFromId(id: string): EntityType {
	const prefix = /^https?:\/\/openalex\.org\/([A-Z])/.exec(id)?.[1]
	switch (prefix) {
		case "W":
			return "works"
		case "A":
