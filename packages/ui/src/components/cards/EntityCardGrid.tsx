import type { EntityType, OpenAlexEntity } from "@academic-explorer/client";
import {
	isAuthor,
	isFunder,
	isInstitution,
...
export const EntityCardGrid: React.FC<EntityCardGridProps> = ({
  ...
  const renderCard = (entity: OpenAlexEntity, index: number) => {
    ...
    if (isWork(entity)) {
      return <WorkCard key={key} work={entity} onNavigate={onNavigate} />;
    }
    if (isAuthor(entity)) {
    ...
    // Fallback to generic entity card
