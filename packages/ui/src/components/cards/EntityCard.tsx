...
export const EntityCard: React.FC<EntityCardProps> = ({
	...
	const isClickable = Boolean(href) && Boolean(onNavigate)
		...
		return (
			<Card
				...
				onClick={(e: React.MouseEvent) => {
					...
					if (onNavigate && e.button === 0 && !e.ctrlKey && !e.metaKey && !e.shiftKey && !e.altKey) {
						e.preventDefault()
			...
			>
				<Stack gap="sm">
					...
					{tags.length > 0 && (
						<Group gap="xs">
							{tags.map((tag, index) => (
								<Badge key={index} color={tag.color || "gray"} variant="dot" size="sm">
									{tag.label}
								</Badge>
							))}
						</Group>
