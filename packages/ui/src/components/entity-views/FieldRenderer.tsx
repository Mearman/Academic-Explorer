...
import { findArrayMatcher, findObjectMatcher, findValueMatcher } from "./matchers/index"
 ...
 */
export const FieldRenderer: React.FC<FieldRendererProps> = ({ fieldName, value, onNavigate }) => {
 ...
 */
export const ValueRenderer: React.FC<{ value: unknown }> = async ({ value }) => {
	const matcher = findValueMatcher({ value, fieldName: "" })
	if (matcher) {
 ...
 */
export const DefaultValueRenderer: React.FC<{ value: unknown }> = ({ value }) => {
	...
	if (typeof value === "object") {
		return <ObjectRenderer obj={value as Record<string, unknown>} fieldName="" />
	}
	...
	return (
		<Stack gap="lg">
			...
			{groupedFields.high.map(([fieldName, value]) => (
				<FieldRenderer key={fieldName} fieldName={fieldName} value={value} onNavigate={onNavigate} />
			))}
