...
export const institutionMatcher: ArrayMatcher = {
	...
	detect(array: unknown[]): boolean {
		...
		const first = array[0]
		return (
			typeof first === "object" && first !== null && "display_name" in first && "country_code" in first
		)
	},
				...
				{institutionArray.map((institution, index) => {
					...
					return (
						<EntityCard
							...
							description={institution.country_code ? institution.country_code.toUpperCase() : undefined}
