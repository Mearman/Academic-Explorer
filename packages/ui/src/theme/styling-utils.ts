...
export function createSectionStyles(tokens: ThemeTokens) {
	return {
		...
		badge: {
			 ...
			 */
			entity: (entityType: keyof typeof tokens.colors) => (theme: MantineTheme) => ({
				backgroundColor: tokens.colors[entityType]?.[1] || theme.colors.gray[1],
			 ...
			 */
			status: (status: "success" | "warning" | "error") => (theme: MantineTheme) => {
				const statusColors = {
					success: {
