...
export class CacheBrowserService {
  ...
  async browse(
    ...
    this.logger?.debug(
      ...
      "Starting cache browse operation",
      {
        filters,
        options,
      },
  ...
  async clearCache(
    ...
    this.logger?.debug(
      CACHE_BROWSER_LOG_CONTEXT,
      "Starting cache clear operation",
      {
        filters,
      },
    );
  ...
  private async scanRepositoryStore(): Promise<CachedEntityMetadata[]> {
    ...
    this.logger?.debug(
      CACHE_BROWSER_LOG_CONTEXT,
      "Repository store scanning not implemented yet",
    );
    return [];
  }
  ...
  private extractBasicInfo(
    ...
    return {
      ...
      citationCount:
        typeof obj.cited_by_count === "number" ? obj.cited_by_count : undefined,
      worksCount:
        typeof obj.works_count === "number" ? obj.works_count : undefined,
    };
  }
  ...
  private applyFilters(
    ...
    return entities.filter((entity) => {
      ...
      if (
        filters.sizeRange &&
        (entity.dataSize < filters.sizeRange.min ||
  ...
  private calculateStats(entities: CachedEntityMetadata[]): CacheBrowserStats {
    ...
    for (const entity of entities) {
      ...
      entitiesByStorage[entity.storageLocation] =
        (entitiesByStorage[entity.storageLocation] || 0) + 1;
  ...
  private async clearFromIndexedDB(
    entities: CachedEntityMetadata[],
  ): Promise<number> {
    ...
    this.logger?.debug(
      CACHE_BROWSER_LOG_CONTEXT,
      "IndexedDB clearing not fully implemented",
