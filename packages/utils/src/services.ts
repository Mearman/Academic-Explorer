...
export function getRelationshipDetectionService(): RelationshipDetectionService {
  ...
  return {
    detectRelationships(_entities: unknown[]): Promise<unknown[]> {
...
export function getGraphDataService(): GraphDataService {
  ...
  return {
    loadGraphData(_entityId: string): Promise<unknown> {
      logger.warn("services", "loadGraphData: Using stub implementation");
      return Promise.resolve(null);
    },

    expandGraph(_nodeId: string, _expansionType: string): Promise<unknown> {
 ...
 * Stub implementation - applications should provide their own
 */
export function useOpenAlexEntity(_entityId: string | undefined): {
  data: unknown;
  isLoading: boolean;
  error: Error | null;
} {
