...
function logValidationSummary({
	...
	logger.debug(LOG_CATEGORY, `   Valid: ${validation.isValid ? "Valid" : "Invalid"}`)
	logger.debug(LOG_CATEGORY, `   Entities validated: ${validation.entitiesValidated}`)
	logger.debug(LOG_CATEGORY, `   Errors found: ${validation.errors.length}`)
	logger.debug(LOG_CATEGORY, `   Warnings found: ${validation.warnings.length}`)
	logger.debug(LOG_CATEGORY, `   Duration: ${validation.performance.durationMs}ms`)
}
...
function logValidationWarnings(validation: IndexValidationResult): void {
	if (validation.warnings.length > 0) {
		...
		for (const warning of validation.warnings) {
			logger.warn(LOG_CATEGORY, `   - ${warning.type}: ${warning.message} (${warning.severity})`)
		}
	}
}
...
export async function exampleGenerateAllIndexes(rootPath: string): Promise<void> {
	try {
		...
		if (result.success) {
			...
			logger.debug(LOG_CATEGORY, `Total entities indexed: ${result.stats.entitiesIndexed}`)
			logger.debug(LOG_CATEGORY, `Total duration: ${result.stats.totalDurationMs}ms`)
			logger.debug(LOG_CATEGORY, "Generated indexes:", Object.keys(result.generatedIndexes))
		} else {
			logger.error(LOG_CATEGORY, "Index generation failed:", result.error?.message)
		}
	} catch (error) {
...
export async function exampleGenerateEntityTypeIndex({
	...
	try {
		...
		logger.debug(LOG_CATEGORY, `Index generated for ${entityType}: ${indexPath}`)
	} catch (error) {
		logger.error(LOG_CATEGORY, `Failed to generate index for ${entityType}:`, error)
	}
}
...
export async function exampleCustomGeneratorWithProgress(rootPath: string): Promise<void> {
	try {
		...
		if (result.success) {
			logger.debug(LOG_CATEGORY, "Index generation completed with progress tracking!")
		} else {
			logger.error(LOG_CATEGORY, "Index generation failed:", result.error?.message)
		}
	} catch (error) {
...
export async function exampleBatchValidation(indexPaths: string[]): Promise<void> {
	...
	for (const result of results) {
		if (result.status === "fulfilled") {
			...
			if (validation.isValid) {
				validCount++
				logger.debug(LOG_CATEGORY, `${indexPath}: Valid (${validation.entitiesValidated} entities)`)
			} else {
				invalidCount++
				logger.warn(LOG_CATEGORY, `${indexPath}: Invalid (${validation.errors.length} errors)`)
			}
		} else {
			invalidCount++
	...
	logger.debug(LOG_CATEGORY, `Validation summary: ${validCount} valid, ${invalidCount} invalid`)
}
