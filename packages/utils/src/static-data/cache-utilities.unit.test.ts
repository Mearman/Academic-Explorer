...
const TEST_AUTHOR_URL = "https://api.openalex.org/authors?filter=orcid:0000-0001-2345-6789"
...
describe("Cache Utilities - Collision Handling", () => {
	...
	describe("mergeCollision", () => {
		...
		it("should update timestamps and collision info correctly", () => {
			...
			expect(merged.collisionInfo!.firstCollision).toBe(TEST_FIRST_COLLISION_TIME) // Unchanged
			expect(merged.collisionInfo!.lastMerge).toBe(currentTime)
			expect(merged.collisionInfo!.totalUrls).toBe(2)
		})
		...
		it("should sort equivalentUrls by recency (most recent first)", () => {
			...
			const timestamps = ["2023-01-01T10:00:00Z", TEST_FIRST_COLLISION_TIME, "2023-01-02T10:00:00Z"]
	...
	describe("reconstructPossibleCollisions", () => {
		...
		it("should include variation with api_key parameter", () => {
			...
			expect(result).toContain(`${TEST_BASE_URL_WITH_FILTER}&${TEST_API_KEY_PARAM}`)
		})
		...
		it("should include variation with mailto parameter", () => {
			...
			expect(result).toContain(`${TEST_BASE_URL_WITH_FILTER}&${TEST_MAILTO_PARAM}`)
		})
		...
		it("should include cursor variation when cursor=* is present", () => {
			...
			const hasCursorVar = result.some((url) => url.includes(TEST_CURSOR_PARAM))
	...
	describe("validateFileEntry", () => {
		...
		it("should invalidate when equivalentUrls[0] !== url", async () => {
			...
			expect(await validateFileEntry(invalidEntry, getCacheFilePath)).toBe(false)
		})
		...
		it("should validate when all URLs normalize to same path", async () => {
			const validEntry: FileEntry = {
				...
				lastRetrieved: TEST_TIMESTAMP,
				contentHash: TEST_CONTENT_HASH,
				equivalentUrls: [TEST_WORKS_URL, TEST_WORKS_URL_WITH_API_KEY, TEST_BASE_URL_WITH_MAILTO],
				urlTimestamps: {
					[TEST_WORKS_URL]: TEST_TIMESTAMP,
...
describe("parseOpenAlexUrl", () => {
	...
	it("should detect encoded ORCID identifiers as entity ids", () => {
		...
		expect(result).not.toBeNull()
		expect(result?.entityId).toBe("https%3A%2F%2Forcid.org%2F0000-0001-2345-6789")
	})

	it("should not assign entity id for non-entity collection endpoints", () => {
		const result = parseOpenAlexUrl("https://api.openalex.org/works/random?per_page=1")
		expect(result).not.toBeNull()
...
describe("getCacheFilePath", () => {
	...
	it("should preserve encoded ORCID identifiers in cache path", async () => {
		...
		expect(cachePath).not.toBeNull()
		expect(cachePath).toContain("authors/https%3A%2F%2Forcid.org%2F0000-0001-2345-6789/queries")
	})
})
