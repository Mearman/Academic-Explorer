...
describe('Filename Encoding/Decoding', () => {
  describe('basic encoding', () => {
    ...
    it('should handle multiple forbidden characters', () => {
      expect(encodeFilename('complex<>?*|/"test'))
        .toBe('complex__3C____3E____3F____2A____7C____2F____22__test');
    });
  ...
  describe('basic decoding', () => {
    ...
    it('should handle multiple encoded characters', () => {
      expect(decodeFilename('complex__3C____3E____3F____2A____7C____2F____22__test'))
        .toBe('complex<>?*|/"test');
    });
  ...
  describe('real-world query examples', () => {
    ...
    it('should handle complex URL-encoded query strings', () => {
      ...
      const encoded = encodeFilename('filter=institutions.country_code%3Aus&sort=cited_by_count%3Adesc');
      const decoded = decodeFilename(encoded);
      // After decoding URL encoding and re-encoding, we get the original unencoded form
      expect(decoded).toBe('filter=institutions.country_code:us&sort=cited_by_count:desc');
    });
  });
  ...
  describe('URL-encoded input handling', () => {
    ...
    it('should handle URL-special characters with unified hex encoding', () => {
      const testCases = [
        { input: 'filter:year:2020', expected: 'filter__3A__year__3A__2020', decoded: 'filter:year:2020' },
        { input: 'key=value', expected: 'key__3D__value', decoded: 'key=value' },
        { input: 'param&other', expected: 'param__26__other', decoded: 'param&other' },
        { input: 'space+here', expected: 'space__2B__here', decoded: 'space+here' },
        { input: 'item,list', expected: 'item__2C__list', decoded: 'item,list' },
        ...
        { input: 'discount%25', expected: 'discount__25__', decoded: 'discount%' }
      ];
    ...
    it('should handle real OpenAlex query with URL encoding', () => {
      ...
      expect(decoded).toBe('filter=author.id:A5023888391&select=id,display_name');
...
describe('Query String Normalization', () => {
  describe('basic parameter stripping', () => {
    ...
    it('should handle cursor parameter specially', () => {
      ...
      expect(normalizeQueryForCaching('?cursor=IlsxNjA5MzcyODAwMDAwLCAnaHR0cHM6Ly9vcGVuYWxleC5vcmcvVzI0ODg0OTk3NjQnXSI=')).toBe('?cursor=*');
      expect(normalizeQueryForCaching('cursor=abc123def456')).toBe('?cursor=*');
    });

    it('should preserve safe parameters', () => {
      expect(normalizeQueryForCaching('?filter=year:2020')).toBe('?filter=year%3A2020');
      expect(normalizeQueryForCaching('?sort=cited_by_count:desc')).toBe('?sort=cited_by_count%3Adesc');
      expect(normalizeQueryForCaching('?search=artificial intelligence')).toBe('?search=artificial+intelligence');
      expect(normalizeQueryForCaching('?per_page=50')).toBe('?per_page=50');
      expect(normalizeQueryForCaching('?group_by=publication_year')).toBe('?group_by=publication_year');
    });
  });

  describe('multiple parameters', () => {
    it('should remove sensitive params while preserving safe ones', () => {
      expect(normalizeQueryForCaching('?filter=year:2020&api_key=secret123&mailto=test@domain.org'))
        .toBe('?filter=year%3A2020');

      expect(normalizeQueryForCaching('?api_key=secret123&filter=year:2020'))
        .toBe('?filter=year%3A2020');

      expect(normalizeQueryForCaching('?filter=year:2020&mailto=test@domain.org&sort=cited_by_count'))
        .toBe('?filter=year%3A2020&sort=cited_by_count');
    });

    it('should handle all three types of parameters together', () => {
      expect(normalizeQueryForCaching('?filter=year:2020&api_key=secret123&mailto=test@domain.org&cursor=abc123'))
        .toBe('?cursor=*&filter=year%3A2020');

      expect(normalizeQueryForCaching('?filter=year:2020&api_key=secret123&mailto=test@domain.org&cursor=*'))
        .toBe('?cursor=*&filter=year%3A2020');
    });

    it('should handle complex parameter combinations', () => {
      const input = '?filter=year:2020&sort=cited_by_count:desc&api_key=sk-123&mailto=user@test.com&per_page=50&cursor=xyz789';
      const expected = '?cursor=*&filter=year%3A2020&per_page=50&sort=cited_by_count%3Adesc';
  ...
  describe('edge cases', () => {
    ...
    it('should handle malformed query strings gracefully', () => {
      ...
      expect(normalizeQueryForCaching('filter=year:2020&api_key=secret&')).toBe('?filter=year%3A2020');
      expect(normalizeQueryForCaching('api_key=secret&filter=year:2020')).toBe('?filter=year%3A2020');
    });

    it('should handle URLs with only sensitive parameters', () => {
      expect(normalizeQueryForCaching('?api_key=secret123')).toBe('');
      expect(normalizeQueryForCaching('?mailto=test@domain.org')).toBe('');
      expect(normalizeQueryForCaching('?api_key=secret&mailto=user@domain.com')).toBe('');
    });

    it('should handle parameters without leading question mark', () => {
      expect(normalizeQueryForCaching('filter=year:2020&api_key=secret123')).toBe('?filter=year%3A2020');
      expect(normalizeQueryForCaching('api_key=secret123&filter=year:2020')).toBe('?filter=year%3A2020');
    });
  });

  describe('real-world query strings', () => {
    it('should handle typical OpenAlex filter queries', () => {
      expect(normalizeQueryForCaching('?filter=institutions.country_code:us&sort=cited_by_count:desc'))
        .toBe('?filter=institutions.country_code%3Aus&sort=cited_by_count%3Adesc');

      expect(normalizeQueryForCaching('?filter=publication_year:2020,authorships.institutions.id:I27837315'))
        .toBe('?filter=publication_year%3A2020%2Cauthorships.institutions.id%3AI27837315');
    });

    it('should handle search queries with special characters', () => {
      expect(normalizeQueryForCaching('?search=artificial intelligence&filter=type:journal-article'))
        .toBe('?filter=type%3Ajournal-article&search=artificial+intelligence');
    });

    it('should handle group_by queries', () => {
      expect(normalizeQueryForCaching('?group_by=publication_year&filter=cited_by_count:>100'))
        .toBe('?filter=cited_by_count%3A%3E100&group_by=publication_year');
    });

    it('should handle pagination with cursor', () => {
      expect(normalizeQueryForCaching('?per_page=200&cursor=*'))
        .toBe('?cursor=*&per_page=200');

      expect(normalizeQueryForCaching('?per_page=200&cursor=IlsxNjA5MzcyODAwMDAwXSI='))
        .toBe('?cursor=*&per_page=200');
    });

    it('should handle select parameter with field lists', () => {
      expect(normalizeQueryForCaching('?select=id,display_name,cited_by_count'))
  ...
  describe('privacy preservation examples', () => {
    it('should remove personal email addresses', () => {
      ...
      testEmails.forEach(email => {
        expect(normalizeQueryForCaching(`?mailto=${email}&filter=year:2020`))
          .toBe('?filter=year%3A2020');
      });
    });
    ...
    it('should remove various API key formats', () => {
      ...
      testKeys.forEach(key => {
        expect(normalizeQueryForCaching(`?api_key=${key}&filter=year:2020`))
          .toBe('?filter=year%3A2020');
      });
    });
    ...
    it('should normalize cursor values consistently', () => {
      ...
      testCursors.forEach(cursor => {
        const result = normalizeQueryForCaching(`?cursor=${cursor}&filter=year:2020`);
...
describe('Cache File Path Generation', () => {
  ...
  describe('entity URLs', () => {
    it('should generate correct paths for single entities', () => {
      expect(getCacheFilePath('https://api.openalex.org/works/W123456', staticDataRoot))
        .toBe('/test/data/openalex/works/W123456.json');

      expect(getCacheFilePath('https://api.openalex.org/authors/A5017898742', staticDataRoot))
        .toBe('/test/data/openalex/authors/A5017898742.json');

      expect(getCacheFilePath('https://api.openalex.org/institutions/I27837315', staticDataRoot))
        .toBe('/test/data/openalex/institutions/I27837315.json');
    });

    it('should handle entity URLs with select parameters', () => {
      expect(getCacheFilePath('https://api.openalex.org/works/W123456?select=id,display_name', staticDataRoot))
        .toBe('/test/data/openalex/works/W123456/queries/select__3D__id__2C__display_name.json');

      expect(getCacheFilePath('https://api.openalex.org/authors/A123?select=id,display_name,orcid', staticDataRoot))
        .toBe('/test/data/openalex/authors/A123/queries/select__3D__id__2C__display_name__2C__orcid.json');
    });
  });

  describe('base collection URLs', () => {
    it('should generate correct paths for base collections', () => {
      expect(getCacheFilePath('https://api.openalex.org/works', staticDataRoot))
        .toBe('/test/data/openalex/works.json');

      expect(getCacheFilePath('https://api.openalex.org/authors', staticDataRoot))
        .toBe('/test/data/openalex/authors.json');

      expect(getCacheFilePath('https://api.openalex.org/institutions', staticDataRoot))
  ...
  describe('query URLs with meaningful parameters', () => {
    it('should generate correct paths for filter queries', () => {
      expect(getCacheFilePath('https://api.openalex.org/works?filter=year:2020', staticDataRoot))
        .toBe('/test/data/openalex/works/queries/filter__3D__year__3A__2020.json');

      expect(getCacheFilePath('https://api.openalex.org/authors?filter=display_name.search:smith', staticDataRoot))
        .toBe('/test/data/openalex/authors/queries/filter__3D__display_name.search__3A__smith.json');
    });

    it('should generate correct paths for search queries', () => {
      expect(getCacheFilePath('https://api.openalex.org/works?search=artificial intelligence', staticDataRoot))
        .toBe('/test/data/openalex/works/queries/search__3D__artificial__2B__intelligence.json');
    });

    it('should generate correct paths for complex queries', () => {
      expect(getCacheFilePath('https://api.openalex.org/works?filter=year:2020&sort=cited_by_count:desc', staticDataRoot))
        .toBe('/test/data/openalex/works/queries/filter__3D__year__3A__2020__26__sort__3D__cited_by_count__3A__desc.json');
    });

    it('should handle cursor parameters correctly', () => {
      expect(getCacheFilePath('https://api.openalex.org/works?filter=year:2020&cursor=*', staticDataRoot))
        .toBe('/test/data/openalex/works/queries/cursor__3D____2A____26__filter__3D__year__3A__2020.json');

      expect(getCacheFilePath('https://api.openalex.org/works?filter=year:2020&cursor=abc123', staticDataRoot))
        .toBe('/test/data/openalex/works/queries/cursor__3D____2A____26__filter__3D__year__3A__2020.json');
    });
  });
  ...
  describe('sensitive parameter handling', () => {
    it('should treat URLs with ONLY sensitive params as base collections', () => {
      ...
      expect(getCacheFilePath('https://api.openalex.org/works?api_key=secret&mailto=user@domain.com', staticDataRoot))
        .toBe('/test/data/openalex/works.json');

      expect(getCacheFilePath('https://api.openalex.org/authors?mailto=joseph@mearman.co.uk', staticDataRoot))
        .toBe('/test/data/openalex/authors.json');

      expect(getCacheFilePath('https://api.openalex.org/institutions?api_key=sk-123456', staticDataRoot))
        .toBe('/test/data/openalex/institutions.json');
    });

    it('should strip sensitive params while preserving meaningful ones', () => {
      expect(getCacheFilePath('https://api.openalex.org/works?filter=year:2020&api_key=secret', staticDataRoot))
        .toBe('/test/data/openalex/works/queries/filter__3D__year__3A__2020.json');

      expect(getCacheFilePath('https://api.openalex.org/authors?mailto=user@domain.com&search=smith', staticDataRoot))
        .toBe('/test/data/openalex/authors/queries/search__3D__smith.json');
    });

    it('should handle mixed sensitive and cursor parameters', () => {
      expect(getCacheFilePath('https://api.openalex.org/works?cursor=abc123&api_key=secret&mailto=user@domain.com', staticDataRoot))
        .toBe('/test/data/openalex/works/queries/cursor__3D____2A__.json');

      expect(getCacheFilePath('https://api.openalex.org/works?cursor=*&api_key=secret', staticDataRoot))
        .toBe('/test/data/openalex/works/queries/cursor__3D____2A__.json');
    });
  });
  ...
  describe('edge cases', () => {
    it('should handle malformed URLs gracefully', () => {
      ...
      expect(getCacheFilePath('https://other-domain.com/works', staticDataRoot)).toBe(null);
    });

    it('should handle URLs without entity types', () => {
      expect(getCacheFilePath('https://api.openalex.org/', staticDataRoot)).toBe(null);
      expect(getCacheFilePath('https://api.openalex.org', staticDataRoot)).toBe(null);
    });

    it('should handle nested paths', () => {
      expect(getCacheFilePath('https://api.openalex.org/works/W123/authors', staticDataRoot))
        .toBe('/test/data/openalex/works/W123/authors.json');
    });

    it('should handle empty static data root', () => {
      expect(getCacheFilePath('https://api.openalex.org/works/W123456', ''))
  ...
  describe('parseOpenAlexUrl integration', () => {
    it('should correctly parse and classify URLs', () => {
      ...
      expect(parseOpenAlexUrl('https://api.openalex.org/works/W123456')).toEqual({
      ...
      expect(parseOpenAlexUrl('https://api.openalex.org/works?filter=year:2020')).toEqual({
        pathSegments: ['works'],
        isQuery: true,
  ...
  describe('real-world examples', () => {
    it('should handle documented OpenAlex API examples correctly', () => {
      ...
      expect(getCacheFilePath('https://api.openalex.org/works?filter=institutions.country_code:us', staticDataRoot))
        .toBe('/test/data/openalex/works/queries/filter__3D__institutions.country_code__3A__us.json');

      expect(getCacheFilePath('https://api.openalex.org/works?group_by=publication_year', staticDataRoot))
        .toBe('/test/data/openalex/works/queries/group_by__3D__publication_year.json');

      expect(getCacheFilePath('https://api.openalex.org/works?search=artificial intelligence&filter=type:journal-article', staticDataRoot))
        .toBe('/test/data/openalex/works/queries/filter__3D__type__3A__journal-article__26__search__3D__artificial__2B__intelligence.json');
    });

    it('should handle problematic URLs that caused issues', () => {
      // URLs that were creating base.json files incorrectly
      expect(getCacheFilePath('https://api.openalex.org/works?api_key=secret123&mailto=researcher@university.edu', staticDataRoot))
        .toBe('/test/data/openalex/works.json'); // Should NOT be works/queries/base.json

      expect(getCacheFilePath('https://api.openalex.org/authors?mailto=you@example.com', staticDataRoot))
...
describe('URL Sanitization', () => {
  describe('sanitizeUrlForCaching', () => {
    it('should remove api_key parameter completely', () => {
      ...
      expect(sanitizeUrlForCaching('?filter=year:2020&api_key=secret')).toBe('?filter=year%3A2020');
    });

    it('should remove mailto parameter completely', () => {
      expect(sanitizeUrlForCaching('?mailto=joseph@mearman.co.uk')).toBe('');
      expect(sanitizeUrlForCaching('mailto=user@domain.org')).toBe('');
      expect(sanitizeUrlForCaching('?filter=year:2020&mailto=test@domain.org')).toBe('?filter=year%3A2020');
    });

    it('should preserve other parameters', () => {
      expect(sanitizeUrlForCaching('?filter=year:2020')).toBe('?filter=year%3A2020');
      expect(sanitizeUrlForCaching('?sort=cited_by_count:desc')).toBe('?sort=cited_by_count%3Adesc');
      expect(sanitizeUrlForCaching('?cursor=abc123def456')).toBe('?cursor=abc123def456');
    });

    it('should handle mixed sensitive and regular parameters', () => {
      expect(sanitizeUrlForCaching('?filter=year:2020&api_key=secret&mailto=test@domain.org'))
        .toBe('?filter=year%3A2020');
    });
  ...
  describe('normalizeQueryForFilename', () => {
    it('should normalize cursor values to asterisk', () => {
      ...
      expect(normalizeQueryForFilename('?cursor=IlsxNjA5MzcyODAwMDAwXSI=')).toBe('?cursor=*');
      expect(normalizeQueryForFilename('cursor=abc123def456')).toBe('?cursor=*');
    });

    it('should sort parameters alphabetically', () => {
      expect(normalizeQueryForFilename('?sort=desc&filter=year:2020'))
        .toBe('?filter=year%3A2020&sort=desc');
      expect(normalizeQueryForFilename('?cursor=abc&filter=year:2020&per_page=50'))
        .toBe('?cursor=*&filter=year%3A2020&per_page=50');
    });

    it('should preserve parameter values', () => {
      expect(normalizeQueryForFilename('?filter=year:2020')).toBe('?filter=year%3A2020');
      expect(normalizeQueryForFilename('?search=artificial intelligence')).toBe('?search=artificial+intelligence');
    });
...
describe('URL Equivalence Comparison', () => {
  describe('areUrlsEquivalentForCaching', () => {
    ...
    it('should ignore sensitive parameters when comparing', () => {
      const url1 = 'https://api.openalex.org/works?filter=year:2020&api_key=secret1';
      const url2 = 'https://api.openalex.org/works?filter=year:2020&api_key=secret2';
    ...
    it('should normalize cursor values during comparison', () => {
      const url1 = 'https://api.openalex.org/works?cursor=abc123&filter=year:2020';
      const url2 = 'https://api.openalex.org/works?cursor=xyz789&filter=year:2020';
    ...
    it('should handle complex real-world URL comparisons', () => {
      const url1 = 'https://api.openalex.org/works?filter=institutions.country_code:us&sort=cited_by_count:desc&api_key=secret&mailto=test@domain.org';
      const url2 = 'https://api.openalex.org/works?mailto=different@email.com&sort=cited_by_count:desc&filter=institutions.country_code:us&api_key=different';
...
describe('Index Format Adapters', () => {
  describe('directoryIndexToUnifiedIndex', () => {
    it('should convert DirectoryIndex to UnifiedIndex format', async () => {
      const { directoryIndexToUnifiedIndex } = await import('./cache-utilities.js');
  ...
  describe('unifiedIndexToDirectoryIndex', () => {
    it('should convert UnifiedIndex to DirectoryIndex format', async () => {
      const { unifiedIndexToDirectoryIndex } = await import('./cache-utilities.js');
