...
const TEST_AUTHORS_URL =
  "https://api.openalex.org/authors?filter=orcid:0000-0001-2345-6789";
const TEST_DOI_FILTER = "filter=doi:10.1234/test";
const TEST_AUTHOR_FILTER = "filter=orcid:0000-0001-2345-6789";
...
const TEST_BASE_URL_WITH_CURSOR = `${TEST_BASE_URL}?${TEST_DOI_FILTER}&cursor=*`;
const TEST_BASE_URL_WITH_CURSOR_VALUE = `${TEST_BASE_URL}?${TEST_DOI_FILTER}&${TEST_CURSOR_PARAM}`;
...
describe("mergeCollision", () => {
  ...
  it("should sort equivalentUrls by recency (most recent first)", () => {
    ...
    const urls = [
      `${TEST_WORKS_URL}&${TEST_API_KEY_PARAM}`,
...
describe("validateFileEntry", () => {
  ...
  it("should validate valid multi-URL entry", () => {
    const validEntry: FileEntry = {
      ...
      urlTimestamps: {
        ...
        [TEST_BASE_URL_WITH_API_KEY]: "2023-01-02T00:00:00Z",
      },
      collisionInfo: {
        mergedCount: 1,
        totalUrls: 2,
  ...
  it("should invalidate when URLs map to different cache paths", () => {
    const invalidEntry: FileEntry = {
      ...
      urlTimestamps: {
        [TEST_WORKS_URL]: TEST_TIMESTAMP,
        [TEST_AUTHORS_URL]: "2023-01-02T00:00:00Z",
      },
  ...
  it("should validate when all URLs normalize to same path", () => {
    const validEntry: FileEntry = {
      ...
      urlTimestamps: {
        [TEST_WORKS_URL]: TEST_TIMESTAMP,
        [`${TEST_WORKS_URL}&${TEST_API_KEY_PARAM}`]: "2023-01-02T00:00:00Z",
        [TEST_BASE_URL_WITH_MAILTO]: "2023-01-03T00:00:00Z",
      },
      collisionInfo: {
        mergedCount: 2,
        totalUrls: 3,
