...
export class SimulationEventEmitter {
	private handlers = new Map<string, Set<SimulationEventHandler<SimulationEvent>>>()
	...
	emitError({ message, context }: { message: string; context?: Record<string, unknown> }): void {
		this.emit({
			type: "error",
			timestamp: Date.now(),
			message,
	...
	emitDebug({ message, context }: { message: string; context?: Record<string, unknown> }): void {
		this.emit({
			type: "debug",
			timestamp: Date.now(),
			message,
