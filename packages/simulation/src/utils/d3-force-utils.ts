...
interface ForceWithStrength {
	strength(): number
	strength(
		strength: number | ((d: D3SimulationNode, i: number, data: D3SimulationNode[]) => number)
	): this
}
...
interface ForceWithDistance {
	distance(): number
	distance(
		distance: number | ((d: D3SimulationNode, i: number, data: D3SimulationNode[]) => number)
	): this
}
...
interface ForceWithRadius {
	radius(): number
	radius(
		radius: number | ((d: D3SimulationNode, i: number, data: D3SimulationNode[]) => number)
	): this
}
...
export function pinNode({ node, x, y }: { node: D3SimulationNode; x?: number; y?: number }): void {
	node.fx = x ?? node.x ?? 0
	node.fy = y ?? node.y ?? 0
}
