...
function generateBadges(coveragePath = './coverage/coverage-summary.json') {
  try {
    ...
    if (providersFiles.length > 0) {
      const providersMetrics = providersFiles.reduce((acc, file) => {
      ...
      if (providersMetrics.count > 0) {
        const providersAvg = Math.round(
          (providersMetrics.statements + providersMetrics.branches +
           providersMetrics.functions + providersMetrics.lines) /
    ...
    if (servicesFiles.length > 0) {
      const servicesMetrics = servicesFiles.reduce((acc, file) => {
        const fileCoverage = coverage[file];
        acc.statements += fileCoverage.statements.pct;
        acc.branches += fileCoverage.branches.pct;
        acc.functions += fileCoverage.functions.pct;
      ...
      if (servicesMetrics.count > 0) {
        const servicesAvg = Math.round(
          (servicesMetrics.statements + servicesMetrics.branches +
           servicesMetrics.functions + servicesMetrics.lines) /
    ...
    const badgeData = {
      ...
      metrics: {
        statements: {
          ...
          url: generateBadgeUrl('statements', `${total.statements.pct}%`, getColor(total.statements.pct))
        },
        branches: {
