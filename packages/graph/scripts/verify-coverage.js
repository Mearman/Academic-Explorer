...
function verifyCoverage(coveragePath = "./coverage/coverage-summary.json") {
	...
	for (const component of PHASE_2_COMPONENTS) {
		...
		const coverageKey = Object.keys(coverage).find(
			(key) => key.endsWith(component) || key.includes(component.replace("src/", ""))
		)
		...
		if (failures.length === 0) {
			...
			console.log(
				`    S: ${componentCoverage.statements.pct}% B: ${componentCoverage.branches.pct}% F: ${componentCoverage.functions.pct}% L: ${componentCoverage.lines.pct}%`
			)
	...
	const phase2Files = allFiles.filter(
		(file) =>
	...
	if (lowCoverageFiles.length > 0) {
		...
		lowCoverageFiles.forEach((file) => {
			...
			console.log(`  ${file}:`)
			console.log(
				`    S: ${fileCoverage.statements.pct}% B: ${fileCoverage.branches.pct}% F: ${fileCoverage.functions.pct}% L: ${fileCoverage.lines.pct}%`
			)
		})
	}
