...
const FIELD_MAPPINGS: FieldMapping = {
	...
	institutions: {
		...
		[CacheContext.SEARCH]: [...CORE_FIELDS, "ror", "country_code", "type", "works_count"],
	},
	publishers: {
		[CacheContext.TRAVERSAL]: [
			...CORE_FIELDS,
	...
	funders: {
		[CacheContext.TRAVERSAL]: [...CORE_FIELDS, "works_api_url", "grants.id"],
		[CacheContext.ANALYSIS]: [
			...CORE_FIELDS,
			"works_count",
			"cited_by_count",
	...
	concepts: {
		...
		[CacheContext.SEARCH]: [...CORE_FIELDS, "description", "level", "works_count"],
	},
	keywords: {
		[CacheContext.TRAVERSAL]: [...CORE_FIELDS, "works_api_url"],
		[CacheContext.ANALYSIS]: [...CORE_FIELDS, "works_count", "cited_by_count", "counts_by_year"],
		[CacheContext.UI_DISPLAY]: [...CORE_FIELDS, "works_count", "cited_by_count"],
		[CacheContext.EXPORT]: [
			"*", // All fields for complete export
		],
...
const RELATIONSHIP_FIELDS: Record<EntityType, string[]> = {
	...
	authors: ["affiliations.institution.id", "concepts.id", "topics.id"],
	sources: ["host_organization.id", "concepts.id", "topics.id"],
	institutions: ["associated_institutions.id", "concepts.id", "topics.id"],
	publishers: ["parent_publisher.id"],
	funders: ["grants.id"],
...
export class ContextualFieldSelector {
	...
	getFieldsForDepth(depth: number, context: CacheContext): string[] {
		...
		const baseFields =
			context === CacheContext.TRAVERSAL ? [...CORE_FIELDS] : this.getRequiredFields("works", context) // Use works as default for mixed entity queries
	...
	getFieldRecommendations(
		...
		const missing = targetFields.filter(
			(field) => field !== "*" && !currentFields.includes(field) && !currentFields.includes("*")
		)
