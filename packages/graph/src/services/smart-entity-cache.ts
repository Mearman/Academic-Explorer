...
export interface EntityDataProvider {
	...
	): Promise<Array<{ id: string; data: EntityData; error?: string }>>
}
...
export class SmartEntityCache {
	...
	async ensureFields(id: string, fields: string[], context: CacheContext = {}): Promise<void> {
		...
		const predictedFields =
			context.prefetch !== false ? this.predictNextFields(id, entityType, context) : []
		...
		try {
			...
			this.stats.correctPredictions += predictedFields.filter(
				(field) => Object.prototype.hasOwnProperty.call(entityData, field) && entityData[field] != null
			).length
		} catch (error) {
	...
	optimizeBatchRequests(requests: FieldRequest[]): BatchRequest[] {
		...
		Array.from(batches.entries()).forEach(([entityType, entityBatch]) => {
			const batchRequests = Array.from(entityBatch.values())
				.map((req) => ({
					id: req.id,
	...
	getStats(): CacheStats {
		...
		const staleCount = entities.reduce((count, entity) => {
			return (
				count +
				Array.from(entity.fieldTimestamps.values()).filter(
					(timestamp) => now - timestamp > this.defaultMaxAge
				).length
			)
		}, 0)
		...
		const avgAge =
			entityAges.length > 0 ? entityAges.reduce((a, b) => a + b, 0) / entityAges.length : 0
		...
		return {
			...
			batchEfficiencyRatio:
				this.stats.batchRequests > 0 ? totalRequests / this.stats.batchRequests : 0,
			predictionAccuracy:
				this.stats.predictedFields > 0 ? this.stats.correctPredictions / this.stats.predictedFields : 0,
	...
	private enforceMaxCacheSize(): void {
		...
		const entities = Array.from(this.cache.entries()).sort(
