...
export interface TaskExecutor {
	(payload: unknown, progressCallback: (event: unknown) => void): Promise<unknown>
}
...
export abstract class BaseExecutionStrategy {
	...
	constructor(
		...
		this.registry = options.executorRegistry || new SimpleTaskExecutorRegistry()
...
export class MainThreadExecutionStrategy extends BaseExecutionStrategy {
	...
	protected async executeTask(task: Task): Promise<void> {
		try {
			...
			if (!executor) {
				throw new Error(`No executor registered for task entityType: ${entityType}`)
			}
...
export async function createExecutionStrategy(
	...
	const { mode = "main-thread", maxConcurrency = 1, executorRegistry } = options
