...
export interface IEntityResolver {
	...
	detectAndResolveEntities(
		ids: string[]
	): Promise<Array<{ id: string; node?: GraphNode; error?: string }>>
	isValidIdentifier(id: string): boolean
	normalizeIdentifier(id: string): string | null
	getSupportedIdentifierTypes(): Array<{
		name: string
...
export class EntityResolver implements IEntityResolver {
	...
	async expandEntity(
		nodeId: string,
		options: Partial<EntityExpansionOptions> = {}
	...
	async detectAndResolveEntity(id: string): Promise<GraphNode> {
		...
		if (!detectionResult) {
			throw new Error(
				`Unrecognized identifier format: "${id}". Supported formats include DOI, ORCID, ROR, ISSN, OpenAlex IDs, and OpenAlex URLs.`
			)
		}
		...
		const context: CacheContext = {
			...
			requiredFields:
				this.fieldSelector?.getFieldsForContext(detectionResult.entityType, {
					contextType: "details",
					usagePattern: "occasional",
				}) || [],
		}
	 ...
	 */
	async detectAndResolveEntities(
		ids: string[]
	): Promise<Array<{ id: string; node?: GraphNode; error?: string }>> {
