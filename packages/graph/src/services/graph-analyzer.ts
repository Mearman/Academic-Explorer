...
export class GraphAnalyzer {
	...
	async calculateNetworkMetrics(
		...
		if (options.computeStatistics && nodeCount <= 5000) {
			metrics.smallWorldness = this.calculateSmallWorldness(
				adjacency,
				pathMetrics.averagePathLength,
				clusteringCoefficient
			)
	...
	async analyzeCollaborations(
		...
		const institutionalCollaboration = await this.analyzeInstitutionalCollaboration(
			authorId,
			collaborationData,
			provider
		)
	...
	async detectTrends(
		...
		const researchTopics = await this.analyzeResearchTopicsEvolution(
			analysisData,
			timeRange,
			provider
		)
	...
	getCacheStats(): {
		...
		return {
			...
			totalSize:
				this.cache.citationMetrics.size +
	...
	private calculateCitationsByYear(
		citingWorks: GraphNode[]
	...
	private async calculateSelfCitations(
		_nodeId: string,
		_citingWorks: GraphNode[],
	...
	private async getTopCitingPapers(
		citingWorks: GraphNode[],
		limit: number
	): Promise<Array<{ nodeId: string; citations: number; title: string }>> {
		// Sort by citation count and return top papers
		return citingWorks.slice(0, limit).map((work) => ({
			nodeId: work.id,
	...
	private async detectCommunities(
		...
		Array<{ id: string; nodes: string[]; size: number; density: number; modularity: number }>
	> {
	...
	private analyzeNetworkRobustness(
		graphData: GraphData,
		adjacency: Map<string, Set<string>>
	): { criticalNodes: string[]; bridgeEdges: string[]; vulnerabilityScore: number } {
	...
	private calculateDegreeDistribution(
		adjacency: Map<string, Set<string>>
	): Array<{ degree: number; frequency: number }> {
	...
	private calculatePathMetrics(_adjacency: Map<string, Set<string>>): {
		diameter: number
		averagePathLength: number
	} {
	...
	private calculateSmallWorldness(
		_adjacency: Map<string, Set<string>>,
		_avgPathLength: number,
		_clusteringCoeff: number
	): number {
		// Small-worldness calculation (Watts-Strogatz)
		return -1 // Placeholder - requires reference random network
	}

	private calculateScaleFreeness(
		_degreeDistribution: Array<{ degree: number; frequency: number }>
	): number {
	...
	private async getCollaborationData(
	...
	): Promise<Array<{ workId: string; collaborators: string[]; year: number; impact: number }>> {
		...
		for (const workId of worksIds) {
			try {
				...
				logger.warn(
					"analysis",
					`Failed to get collaboration data for work ${workId}`,
					{ error },
					"GraphAnalyzer"
				)
			}
	...
	private async buildCollaborationNetwork(
		...
		const collaboratorMap = new Map<
			string,
			{ collaborations: number; sharedWorks: string[]; totalImpact: number }
		>()
	...
	private async analyzeInstitutionalCollaboration(
		_authorId: string,
		_collaborationData: Array<{
			workId: string
			collaborators: string[]
			year: number
			impact: number
		...
		primaryInstitution?: string
		institutionalDiversity: number
		crossInstitutionalRate: number
		topInstitutions: Array<{ institutionId: string; collaborations: number; impactScore: number }>
	}> {
	...
	private async analyzeDisciplinaryReach(
		_worksIds: string[],
		_provider: GraphDataProvider
	): Promise<{
		primaryFields: string[]
	...
	private analyzeCollaborationTemporalPatterns(
		collaborationData: Array<{
			workId: string
			collaborators: string[]
			year: number
		...
		const yearlyPatterns = new Map<
			number,
			{ newCollabs: Set<string>; repeatCollabs: Set<string>; allCollabs: Set<string> }
		>()
		...
		collaborationData
			.sort((a, b) => a.year - b.year)
			.forEach((collab) => {
				const yearData = yearlyPatterns.get(collab.year) || {
					newCollabs: new Set(),
					repeatCollabs: new Set(),
					allCollabs: new Set(),
				}
		...
		return Array.from(yearlyPatterns.entries()).map(([year, data]) => ({
			year,
	...
	private calculateCollaborationCentrality(collaborationNetwork: {
		directCollaborators: Array<{
			nodeId: string
			collaborations: number
			sharedWorks: string[]
		...
		collaborationStrength: number
	}): { degreeCentrality: number; betweennessCentrality: number; brokerage: number } {
		return {
	...
	private async analyzeCollaborationImpact(
		collaborationData: Array<{
			workId: string
			collaborators: string[]
			year: number
		...
		const soloWorkImpact =
			soloWorks.reduce((sum, w) => sum + w.impact, 0) / Math.max(soloWorks.length, 1)
		const collaborativeWorkImpact =
			collabWorks.reduce((sum, w) => sum + w.impact, 0) / Math.max(collabWorks.length, 1)
	...
	private calculatePublicationTrends(
		...
		const recentAvg =
			recentYears.reduce((sum, y) => sum + y.count, 0) / Math.max(recentYears.length, 1)
		const earlierAvg =
			earlierYears.reduce((sum, y) => sum + y.count, 0) / Math.max(earlierYears.length, 1)
	...
	private async analyzeResearchTopicsEvolution(
		_analysisData: GraphData,
		timeRange: TimeRange,
		_provider: GraphDataProvider
	): Promise<{
		currentTopics: Array<{
		...
		topicTransitions: Array<{ from: string; to: string; transitionYear: number; strength: number }>
		emergingAreas: string[]
	}> {
	...
	private async calculateImpactEvolution(
		analysisData: GraphData,
		timeRange: TimeRange,
		_provider: GraphDataProvider
	): Promise<{
		...
		const citationThreshold =
			analysisData.nodes.map((n) => this.extractCitationCount(n)).sort((a, b) => b - a)[
				Math.floor(analysisData.nodes.length * 0.1)
			] || 0
	...
	private analyzeNetworkEvolution(
		_analysisData: GraphData,
	...
	private generateFutureProjections(
		...
		emergingTopicAlignment: number
		confidence: number
	} {
		// Simple linear projection based on recent trends
		const recentOutput = publicationTrends.yearlyOutput.slice(-3).map((y) => y.count)
		const avgRecentOutput =
			recentOutput.reduce((sum: number, count: number) => sum + count, 0) / recentOutput.length

		const recentImpact = impactEvolution.impactByYear.slice(-3).map((y) => y.totalCitations)
		const avgRecentImpact =
			recentImpact.reduce((sum: number, citations: number) => sum + citations, 0) / recentImpact.length
		...
		Object.entries(metrics).forEach(([nodeId, metric]) => {
			...
			switch (analysisType) {
				case "citation": {
					...
					score =
						citationMetric.totalCitations * 0.5 +
						citationMetric.hIndex * 10 +
						citationMetric.recentCitations * 0.3
					break
				}
				case "collaboration": {
					const collabMetric = metric as CollaborationMetrics
					score =
						collabMetric.collaborationNetwork.directCollaborators.length * 2 +
				...
				case "trends": {
					...
					score =
						trendMetric.publicationTrends.yearlyOutput.reduce((sum, y) => sum + y.count, 0) +
