...
describe('Example: Entity Detection and Type Resolution', () => {
    ...
    it('demonstrates DOI-based detection for works', async () => {
      ...
      for (const doi of doiFormats) {
        try {
          ...
          const doiIdentifier = entity.externalIds.find(id => id.type === 'doi');
    ...
    it('demonstrates ORCID-based detection for authors', async () => {
      ...
      for (const orcid of orcidFormats) {
        try {
          ...
          const orcidIdentifier = entity.externalIds.find(id => id.type === 'orcid');
    ...
    it('demonstrates ROR-based detection for institutions', async () => {
      ...
      for (const ror of rorFormats) {
        try {
          ...
          const rorIdentifier = entity.externalIds.find(id => id.type === 'ror');
    ...
    it('demonstrates handling of numeric-only identifiers', async () => {
      ...
      for (const numericId of numericIds) {
        try {
          ...
          expect(['works', 'authors', 'sources', 'institutions', 'topics', 'publishers', 'funders', 'concepts'])
          ...
          expect((error as Error).message).toContain('Cannot detect entity type');
        }
      }
    });
  ...
  describe('Best Practices for Entity Detection', () => {
    ...
    it('demonstrates type-safe entity detection patterns', async () => {
      ...
      const processEntity = async (identifier: string): Promise<{
        type: EntityType;
        label: string;
      ...
      results.forEach((result, index) => {
        ...
        console.log(`Entity ${testIds[index]}: ${result.type} - "${result.label}" (External IDs: ${result.hasExternalIds})`);
      });
