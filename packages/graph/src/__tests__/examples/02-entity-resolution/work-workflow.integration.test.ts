...
class PublicationMockClient {
  private workDatabase = new Map([
    ...
    [
      "W3048589302",
      {
        ...
        topics: [{ id: "T11234", display_name: "AI Ethics", score: 0.91 }],
      },
    ],
    [
      "W1234567890",
  ...
  async getInstitution(): Promise<Record<string, unknown>> {
    return {};
  }
    ...
    if (params.filter) {
      ...
      if (filter.referenced_works) {
        ...
        if (referencingWork?.referenced_works) {
          return {
            results: (
              referencingWork.referenced_works
          ...
          results: Array.from(this.workDatabase.values())
            .filter((work) =>
              work.authorships?.some(
                (auth: any) => auth.author.id === authorId,
              ),
            )
            .slice(0, (params.per_page as number) || 10),
        };
...
describe("Example: Work-Centered Research Workflows", () => {
  ...
  describe("Publication Analysis", () => {
    it("demonstrates detailed publication profiling", async () => {
      ...
      console.log(
        `Authors: ${profile.authorship.author_count}, Citations: ${profile.publication.citation_count}`,
      );
    });
    ...
    it("demonstrates publication timeline and trend analysis", async () => {
      ...
      const years = timeline.temporal_distribution.map(
  ...
  describe("Citation Network Analysis", () => {
    ...
    it("demonstrates citation influence measurement", async () => {
      ...
      expect(
        influence.field_influence.interdisciplinary_impact,
  ...
  describe("Research Network Construction", () => {
    ...
    it("demonstrates bibliographic coupling analysis", async () => {
      ...
      const coupling = await analyzeBibliographicCoupling(provider, [
        workId1,
        workId2,
      ]);
      ...
      if (coupling.coupling_pairs.length > 0) {
        ...
        expect(
          coupling.coupling_pairs[0].shared_references.length,
        ).toBeGreaterThan(0);
      }
    });
  ...
  describe("Literature Review Support", () => {
    it("demonstrates systematic review preparation", async () => {
      ...
      const reviewPrep = await prepareSystematicReview(
        provider,
        seedPapers,
        researchQuestion,
      );
      ...
      expect(
        reviewPrep.literature_landscape.core_papers.length,
      ).toBeGreaterThan(0);
    });
    ...
    it("demonstrates gap analysis in literature", async () => {
      ...
      const gapAnalysis = await analyzeLiteratureGaps(
        provider,
        researchArea,
        knownWorks,
      );
  ...
  async function createPublicationProfile(
    ...
    const authors = expansion.nodes.filter(
      (node) => node.entityType === "authors",
    );
    const correspondingAuthors =
      workData.authorships?.filter((auth: any) => auth.is_corresponding) || [];
    ...
    return {
      ...
      authorship: {
        ...
        corresponding_authors: correspondingAuthors.map(
          (auth: any) => auth.author.display_name,
        ),
        institutional_diversity:
          new Set(
            workData.authorships?.flatMap(
              (auth: any) =>
      ...
      research_context: {
        primary_topics:
          workData.topics?.map((topic: any) => topic.display_name) || [],
        research_fields:
          workData.concepts?.map((concept: any) => concept.display_name) || [],
        sdg_alignment: workData.sustainable_development_goals || [],
      },
      publication_venue: {
        source_name:
          workData.primary_location?.source?.display_name || "Unknown",
        source_type: "journal",
        is_oa_venue: workData.primary_location?.source?.is_oa || false,
      },
      impact_metrics: {
        citations_per_year:
          workData.cited_by_count /
          Math.max(1, 2024 - workData.publication_year),
  ...
  async function analyzeResearchTopics(
    provider: OpenAlexGraphProvider,
    workId: string,
  ) {
    const work = await provider.fetchEntity(workId);
    ...
    return {
      ...
      interdisciplinary_nature: {
        ...
        field_diversity_score:
          topics.length > 0 ? Math.min(topics.length / 5, 1) : 0,
      },
      topic_hierarchy: {
        domain: "Computer Science",
        field: primaryTopic.display_name?.includes("Language")
  ...
  async function analyzePublicationTimeline(
    provider: OpenAlexGraphProvider,
    workIds: string[],
  ) {
    const works = await Promise.all(
      workIds.map((id) => provider.fetchEntity(id)),
    );

    const timelineData = works
      .map((work) => ({
    ...
    const yearlyStats = timelineData.reduce(
      (acc, work) => {
        if (!acc[work.year]) {
    ...
    return {
      temporal_distribution: Object.entries(yearlyStats).map(
        ([year, stats]) => ({
          year: parseInt(year),
          ...stats,
        }),
      ...
      impact_trajectory: {
        early_citations: timelineData
          .slice(0, 2)
          .reduce((sum, work) => sum + work.citations, 0),
        recent_citations: timelineData
          .slice(-2)
          .reduce((sum, work) => sum + work.citations, 0),
        citation_velocity:
          timelineData.length > 0
            ? timelineData.reduce((sum, work) => sum + work.citations, 0) /
              timelineData.length
            : 0,
      },
    };
  ...
  async function analyzeBackwardReferences(
    provider: OpenAlexGraphProvider,
    workId: string,
  ) {
    const work = await provider.fetchEntity(workId);
  ...
  async function measureCitationInfluence(
    provider: OpenAlexGraphProvider,
    workId: string,
  ) {
    const work = await provider.fetchEntity(workId);
  ...
  async function buildCoCitationNetwork(
    provider: OpenAlexGraphProvider,
    workIds: string[],
  ) {
    const nodes = [];
  ...
  async function analyzeBibliographicCoupling(
    provider: OpenAlexGraphProvider,
    workIds: string[],
  ) {
    const works = await Promise.all(
    ...
    const references = works.map(
      (work) => (work.entityData as any)?.referenced_works || [],
    );
      ...
      coupling_pairs: [
        {
          ...
          coupling_strength:
            sharedRefs.length /
            Math.max(references[0]?.length || 1, references[1]?.length || 1),
        },
      ],
