  ...
  async works(params: Record<string, unknown>): Promise<{ results: Record<string, unknown>[] }> {
    ...
    if (params.filter && typeof params.filter === 'object') {
      ...
      if (filter.author?.id) {
        ...
        return { results: authorWorks.slice(0, params.per_page as number || 10) };
      }
    }
...
describe('Example: Author-Centered Research Workflows', () => {
  ...
  describe('Academic Profile Building', () => {
    it('demonstrates building a complete researcher profile', async () => {
      ...
      console.log('Research areas:', profile.research_areas.map(ra => ra.topic));
    });
    ...
    it('demonstrates impact metrics calculation', async () => {
      ...
      expect(metrics.productivity_metrics.total_publications).toBeGreaterThan(0);
      expect(metrics.open_access_metrics.oa_percentage).toBeGreaterThanOrEqual(0);
      expect(metrics.open_access_metrics.oa_percentage).toBeLessThanOrEqual(100);
    });
    ...
    it('demonstrates career timeline construction', async () => {
      ...
      const recentStage = timeline.career_stages[timeline.career_stages.length - 1];
  ...
  describe('Collaboration Network Analysis', () => {
    it('demonstrates co-authorship network construction', async () => {
      ...
      const network = await buildCollaborationNetwork(provider, authorId, { depth: 2 });
    ...
    it('demonstrates institutional collaboration patterns', async () => {
      ...
      const collaboration = await analyzeInstitutionalCollaboration(provider, workId);
      ...
      expect(collaboration.participating_institutions.length).toBeGreaterThan(1);

      if (collaboration.collaboration_type === 'international') {
        const countries = new Set(collaboration.participating_institutions.map(
    ...
    it('demonstrates research community identification', async () => {
      ...
      expect(communities.primary_community.community_strength).toBeGreaterThan(0);
    });
  });
  ...
  describe('Research Impact Analysis', () => {
    ...
    it('demonstrates cross-disciplinary influence tracking', async () => {
      ...
      const influence = await trackCrossDisciplinaryInfluence(provider, authorId);
      ...
      if (influence.influenced_fields.length > 1) {
        expect(influence.influenced_fields.some(
  ...
  async function buildResearcherProfile(provider: OpenAlexGraphProvider, authorId: string) {
    ...
    return {
      ...
      affiliations: (authorData.last_known_institutions || []).map((inst: any) => ({
  ...
  async function calculateImpactMetrics(provider: OpenAlexGraphProvider, authorId: string) {
    const authorNode = await provider.fetchEntity(authorId);
    const authorData = authorNode.entityData as any;
    ...
    const totalCitations = works.reduce((sum, work) =>
      sum + ((work.entityData as any)?.cited_by_count || 0), 0
    );
    ...
    return {
      citation_metrics: {
        ...
        avg_citations_per_paper: works.length > 0 ? totalCitations / works.length : 0
      },
      productivity_metrics: {
  ...
  async function buildCareerTimeline(provider: OpenAlexGraphProvider, authorId: string) {
    ...
    works.forEach(work => {
      ...
      yearlyData.get(year).citations += (work.entityData as any)?.cited_by_count || 0;
    });
  ...
  async function analyzeInstitutionalCollaboration(provider: OpenAlexGraphProvider, workId: string) {
    ...
    const countries = new Set(participating_institutions.map(inst => inst.country));
    ...
    return {
      ...
      lead_institution: participating_institutions.find(inst => inst.corresponding_authors > 0) || participating_institutions[0]
    };
  }

  async function identifyResearchCommunities(provider: OpenAlexGraphProvider, authorId: string) {
    const authorNode = await provider.fetchEntity(authorId);
    const authorData = authorNode.entityData as any;

    // Use topics as proxy for research communities
    const topics = authorData.topics || [];
    const primaryTopic = topics[0] || { display_name: 'Machine Learning', score: 95 };
    ...
    return {
      ...
      interdisciplinary_connections: topics.length > 2 ? ['AI Ethics', 'Healthcare Technology'] : []
    };
  }

  async function analyzeCitationPatterns(provider: OpenAlexGraphProvider, workId: string) {
    const work = await provider.fetchEntity(workId);
    const workData = work.entityData as any;
    ...
    return {
      ...
      citing_fields: ['Machine Learning', 'Medical Informatics', 'Computer Vision'],
  ...
  async function trackCrossDisciplinaryInfluence(provider: OpenAlexGraphProvider, authorId: string) {
    const authorNode = await provider.fetchEntity(authorId);
    const authorData = authorNode.entityData as any;
