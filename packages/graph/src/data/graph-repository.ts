...
export class GraphRepository {
  ...
  async merge(snapshotIds: string[], name?: string): Promise<GraphSnapshot | null> {
    ...
    const merged = await this.createSnapshot(
  ...
  async getStorageInfo(): Promise<{
    ...
    return {
      ...
      oldestSnapshot: snapshots.length > 0 ? Math.min(...snapshots.map(s => s.timestamp)) : undefined,
      newestSnapshot: snapshots.length > 0 ? Math.max(...snapshots.map(s => s.timestamp)) : undefined,
    };
  }
...
export function createGraphRepository(storageType: 'indexeddb' | 'localstorage' = 'indexeddb'): GraphRepository {
