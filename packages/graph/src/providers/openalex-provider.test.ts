...
const createMockWork = (id = 'W123456789') => ({
  id,
  ...
  authorships: [
    {
      author: {
        ...
        ids: {
          ...
          openalex: 'https://openalex.org/A111111111',
        },
      },
    },
  ],
  primary_location: {
    source: {
      ...
      ids: {
        issn_l: '1234-5678',
        openalex: 'https://openalex.org/S222222222',
      },
    },
  },
});

const createMockAuthor = (id = 'A111111111') => ({
  id,
  ...
  last_known_institutions: [
    {
      ...
      ids: {
        ...
        openalex: 'https://openalex.org/I333333333',
      },
    },
  ],
});

const createMockSource = (id = 'S222222222') => ({
  id,
...
const createMockInstitution = (id = 'I333333333') => ({
  id,
...
const createMockTopic = (id = 'T444444444') => ({
  id,
...
describe('OpenAlexGraphProvider', () => {
  ...
  describe('Entity Type Detection', () => {
    it('should detect entity types from OpenAlex IDs correctly', async () => {
      const testCases = [
        { id: 'W123456789', type: 'works' },
        { id: 'A111111111', type: 'authors' },
        { id: 'S222222222', type: 'sources' },
        { id: 'I333333333', type: 'institutions' },
        { id: 'T444444444', type: 'topics' },
        { id: 'P555555555', type: 'publishers' },
        { id: 'F666666666', type: 'funders' },
        { id: 'C777777777', type: 'concepts' },
      ];
  ...
  describe('fetchEntity Method', () => {
    it('should fetch works correctly', async () => {
      const mockWork = createMockWork('W123456789');
      mockClient.getWork.mockResolvedValue(mockWork);

      const node = await provider.fetchEntity('W123456789');

      expect(mockClient.getWork).toHaveBeenCalledWith('W123456789');
      expect(node.id).toBe('W123456789');
      expect(node.entityType).toBe('works');
      expect(node.entityId).toBe('W123456789');
    ...
    it('should fetch authors correctly', async () => {
      const mockAuthor = createMockAuthor('A111111111');
      mockClient.getAuthor.mockResolvedValue(mockAuthor);

      const node = await provider.fetchEntity('A111111111');

      expect(mockClient.getAuthor).toHaveBeenCalledWith('A111111111');
      expect(node.id).toBe('A111111111');
    ...
    it('should fetch sources correctly', async () => {
      const mockSource = createMockSource('S222222222');
      mockClient.getSource.mockResolvedValue(mockSource);

      const node = await provider.fetchEntity('S222222222');

      expect(mockClient.getSource).toHaveBeenCalledWith('S222222222');
      expect(node.id).toBe('S222222222');
      expect(node.entityType).toBe('sources');
      expect(node.label).toBe('Test Journal Name');
    });

    it('should fetch institutions correctly', async () => {
      const mockInstitution = createMockInstitution('I333333333');
      mockClient.getInstitution.mockResolvedValue(mockInstitution);

      const node = await provider.fetchEntity('I333333333');

      expect(mockClient.getInstitution).toHaveBeenCalledWith('I333333333');
      expect(node.id).toBe('I333333333');
    ...
    it('should fetch topics via generic get method', async () => {
      const mockTopic = createMockTopic('T444444444');
      mockClient.get.mockResolvedValue(mockTopic);

      const node = await provider.fetchEntity('T444444444');

      expect(mockClient.get).toHaveBeenCalledWith('topics', 'T444444444');
      expect(node.id).toBe('T444444444');
    ...
    it('should track request statistics on success', async () => {
      ...
      await provider.fetchEntity('W123456789');
    ...
    it('should track request statistics on failure', async () => {
      ...
      await expect(provider.fetchEntity('W123456789')).rejects.toThrow('API Error');
    ...
    it('should emit error events on failure', async () => {
      ...
      await expect(provider.fetchEntity('W123456789')).rejects.toThrow('API Timeout');
  ...
  describe('searchEntities Method', () => {
    it('should search across multiple entity types', async () => {
      const mockWorksResults = {
        results: [createMockWork('W100'), createMockWork('W200')],
      };
      const mockAuthorsResults = {
        results: [createMockAuthor('A100'), createMockAuthor('A200')],
      };
    ...
    it('should handle search errors gracefully', async () => {
      mockClient.works.mockResolvedValue({ results: [createMockWork('W100')] });
  ...
  describe('expandEntity Method', () => {
    it('should expand works with authors and sources', async () => {
      ...
      const expansion = await provider.expandEntity('W123456789', options);
      ...
      expect(authoredEdge!.source).toBe('A111111111');
      expect(authoredEdge!.target).toBe('W123456789');
      ...
      expect(publishedInEdge!.source).toBe('W123456789');
      expect(publishedInEdge!.target).toBe('S222222222');

      // Check metadata
      expect(expansion.metadata.expandedFrom).toBe('W123456789');
    ...
    it('should expand authors with their works', async () => {
      ...
      const expansion = await provider.expandEntity('A111111111', options);

      expect(mockClient.works).toHaveBeenCalledWith({
        filter: { author: { id: 'A111111111' } },
        per_page: 5,
      ...
      expansion.edges.forEach(edge => {
        ...
        expect(edge.source).toBe('A111111111');
      });
    });
    ...
    it('should expand sources with their works', async () => {
      ...
      const expansion = await provider.expandEntity('S222222222', {});

      expect(mockClient.works).toHaveBeenCalledWith({
        filter: { primary_location: { source: { id: 'S222222222' } } },
        per_page: 10,
        sort: 'publication_year:desc',
      });
    ...
    it('should expand institutions with their authors', async () => {
      ...
      const expansion = await provider.expandEntity('I333333333', {});

      expect(mockClient.authors).toHaveBeenCalledWith({
        filter: { last_known_institutions: { id: 'I333333333' } },
        per_page: 10,
      });
      ...
      expansion.edges.forEach(edge => {
        ...
        expect(edge.target).toBe('I333333333');
      });
    });
    ...
    it('should expand topics with their works', async () => {
      ...
      const expansion = await provider.expandEntity('T444444444', {});

      expect(mockClient.works).toHaveBeenCalledWith({
        filter: { topics: { id: 'T444444444' } },
        per_page: 10,
        sort: 'publication_year:desc',
      });
    ...
    it('should handle expansion errors gracefully', async () => {
      ...
      const expansion = await provider.expandEntity('A111111111', {});

      expect(expansion.nodes).toHaveLength(0);
      expect(expansion.edges).toHaveLength(0);
      expect(consoleSpy).toHaveBeenCalledWith(
        'Failed to expand author A111111111:',
        expect.any(Error)
      );
    ...
    it('should handle works without authorships', async () => {
      ...
      const expansion = await provider.expandEntity('W123456789', {});
    ...
    it('should respect limit parameter in expansion', async () => {
      ...
      const expansion = await provider.expandEntity('W123456789', { limit: 5 });
    ...
    it('should handle unknown entity types in expansion', async () => {
      // Should throw error because detectEntityType fails for unknown types
  ...
  describe('Error Handling', () => {
    it('should handle API timeouts', async () => {
      ...
      await expect(provider.fetchEntity('W123456789')).rejects.toThrow('Request timeout');
    ...
    it('should handle malformed API responses', async () => {
      ...
      await expect(provider.fetchEntity('W123456789')).rejects.toThrow();
    });

    it('should handle network errors', async () => {
      mockClient.getWork.mockRejectedValue(new Error('Network unreachable'));

      await expect(provider.fetchEntity('W123456789')).rejects.toThrow('Network unreachable');
    });
    ...
    it('should handle 404 responses', async () => {
      ...
      await expect(provider.fetchEntity('W123456789')).rejects.toThrow('Not Found');
    });
    ...
    it('should handle rate limiting errors', async () => {
      ...
      await expect(provider.fetchEntity('W123456789')).rejects.toThrow('Rate limit exceeded');
    });
  });
  ...
  describe('Performance Timing Tests', () => {
    it('should track response times correctly', async () => {
      ...
      const requestPromise = provider.fetchEntity('W123456789');
    ...
    it('should update average response time correctly across multiple requests', async () => {
      ...
      // Make three requests with simulated timing
      await provider.fetchEntity('W1');
      await provider.fetchEntity('W2');
      await provider.fetchEntity('W3');
    ...
    it('should handle empty authorships in work expansion', async () => {
      const mockWorkWithNullAuthor = {
        ...
        authorships: [
          ...
          { author: { id: 'A111111111', display_name: 'Valid Author' } },
        ],
      };
      ...
      expect(expansion.nodes).toHaveLength(1);
      expect(expansion.edges).toHaveLength(1);
      expect(expansion.nodes[0].id).toBe('A111111111');
    });
  });
  ...
  describe('Provider Integration', () => {
    it('should work with different mock client implementations', async () => {
      ...
      const node = await altProvider.fetchEntity('W123456789');
      expect(node.id).toBe('W123456789');
    ...
    it('should maintain statistics across different operations', async () => {
      ...
      await provider.expandEntity('W123456789', {});
