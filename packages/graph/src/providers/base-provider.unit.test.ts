...
describe("GraphDataProvider", () => {
  ...
  describe("Abstract Method Implementations", () => {
    ...
    it("should handle expandEntity with depth alias", async () => {
      ...
      const result = await provider.expandEntity("A5017898742", options);
  ...
  describe("Batch Operations", () => {
    it("should implement fetchEntities with sequential calls", async () => {
      const ids = ["W2741809807", "W3126653431", "W2963537269"];
      const results = await provider.fetchEntities(ids);

      expect(results).toHaveLength(3);
      expect(results[0].entityId).toBe("W2741809807");
      expect(results[1].entityId).toBe("W3126653431");
      expect(results[2].entityId).toBe("W2963537269");
