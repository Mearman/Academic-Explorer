...
function isForceWithStrength(force: unknown): force is { strength: (value?: number) => unknown } {
	return (
		typeof force === "object" &&
		force !== null &&
		"strength" in force &&
...
export function createForceSimulationExecutor() {
	...
	return async (
		...
		switch (task.entityType) {
			...
			default:
				throw new Error(`Unknown force simulation task entityType: ${task.entityType}`)
		}
	}

	async function handleStart(task: ForceSimulationTask, progressCallback: (event: unknown) => void) {
		if (!task.nodes || !task.links) {
			throw new Error("Nodes and links are required for FORCE_SIMULATION_START")
		}
	...
	async function handleReheat(
		...
		if (!task.nodes || !task.links) {
			throw new Error("Nodes and links are required for FORCE_SIMULATION_REHEAT")
		}
