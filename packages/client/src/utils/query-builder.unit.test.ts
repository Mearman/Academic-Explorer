...
import type { WorksFilters, AuthorsFilters, EntityFilters } from "../types";
import type { SortOptions } from "./query-builder";
...
describe("QueryBuilder", () => {
  ...
  describe("addSearch", () => {
    ...
    it("should support method chaining", () => {
      const result = queryBuilder.addSearch(
        "display_name.search",
        "machine learning",
      );
      expect(result).toBe(queryBuilder);
...
describe("buildFilterString", () => {
  it("should return empty string for empty filters", () => {
    ...
    expect(
      buildFilterString(undefined as Partial<EntityFilters> | undefined),
    ).toBe("");
  });
...
describe("buildSortString", () => {
  ...
  it("should handle array of sort options", () => {
    ...
    expect(result).toBe(
      "publication_year:desc,cited_by_count:desc,display_name:asc",
    );
  });
...
describe("validateDateRange", () => {
  ...
  it("should reject missing dates", () => {
    ...
    result = validateDateRange(null, undefined);
    expect(result.isValid).toBe(false);
    expect(result.error).toContain("Both from and to dates must be provided");
  });
...
describe("Factory Functions", () => {
  describe("createWorksQuery", () => {
    ...
    it("should support method chaining", () => {
      const query = createWorksQuery({ publication_year: 2023 }).addFilter(
        "is_oa",
        true,
      );
...
describe("Constants", () => {
  ...
  describe("SELECT_PRESETS", () => {
    it("should contain expected select field presets", () => {
      ...
      expect(SELECT_PRESETS.BASIC).toEqual([
        "id",
        "display_name",
        "cited_by_count",
      ]);
...
describe("Edge Cases and Error Conditions", () => {
  describe("QueryBuilder edge cases", () => {
    ...
    it("should handle very large numbers", () => {
      const query = new QueryBuilder<WorksFilters>().addFilter(
        "cited_by_count",
