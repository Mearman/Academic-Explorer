...
describe("StatisticsApi", () => {
  ...
  describe("getDatabaseStats", () => {
    it("should get comprehensive database statistics with entity counts", async () => {
      ...
      const mockCitationsResponse: OpenAlexResponse<{
      ...
      const mockTemporalResponse: OpenAlexResponse<{
        ...
        group_by: [
          { key: "2020", key_display_name: "2020", count: 50000 },
          { key: "2021", key_display_name: "2021", count: 52000 },
          { key: "2022", key_display_name: "2022", count: 55000 },
        ],
      };

      const mockGeoResponse: OpenAlexResponse<{
        group_by?: Array<{
          key: string;
          key_display_name: string;
          count: number;
      ...
      vi.mocked(mockClient.getResponse).mockImplementation(
        (entityType, params) => {
    ...
    it("should handle entity count errors gracefully", async () => {
      ...
      vi.mocked(mockClient.getResponse)
        .mockResolvedValueOnce({
          results: [],
          meta: { count: 100, db_response_time_ms: 20, page: 1, per_page: 1 },
        })
        .mockRejectedValueOnce(new Error("API Error"))
        .mockResolvedValue({
          results: [],
          meta: { count: 50, db_response_time_ms: 20, page: 1, per_page: 1 },
        });
  ...
  describe("getEntityAnalytics", () => {
    it("should get detailed analytics for works entity type", async () => {
      const mockDistributionResponse: OpenAlexResponse<{
        cited_by_count?: number;
      }> = {
        results: [
          { cited_by_count: 1000 },
      ...
      const mockTrendResponse: OpenAlexResponse<{
        group_by?: Array<{ key: string; count: number }>;
      }> = {
        results: [],
        meta: { count: 50000, db_response_time_ms: 35, page: 1, per_page: 1 },
        group_by: [
          { key: "2020", key_display_name: "2020", count: 10000 },
          { key: "2021", key_display_name: "2021", count: 12000 },
          { key: "2022", key_display_name: "2022", count: 15000 },
          { key: "2023", key_display_name: "2023", count: 13000 },
        ],
      };

      const mockCollabResponse: OpenAlexResponse<{
    ...
    it("should handle entity types without collaboration metrics", async () => {
      ...
      const mockTrendResponse: OpenAlexResponse<{
        ...
        group_by: [{ key: "2023", key_display_name: "2023", count: 1000 }],
      };
  ...
  describe("getImpactMetrics", () => {
    it("should get impact metrics for entity type", async () => {
      ...
      const mockFieldResponse: OpenAlexResponse<{
        group_by?: Array<{
          key: string;
          key_display_name: string;
          count: number;
        ...
        group_by: [
          ...
          },
          {
            ...
            count: 30000,
            cited_by_count: 600000,
          },
        ],
      };

      const mockTemporalImpactResponse: OpenAlexResponse<{
  ...
  describe("getComparativeStats", () => {
    ...
    it("should handle unsupported grouping gracefully", async () => {
      ...
      const mockEmptyResponse: OpenAlexResponse<{
        group_by?: Array<{
          key: string;
          key_display_name: string;
          count: number;
      ...
      await expect(
        statisticsApi.getComparativeStats("works", "invalid_field"),
      ).rejects.toThrow("Grouping not supported for works by invalid_field");
    });
  });
