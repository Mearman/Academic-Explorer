...
describe("SamplingApi", () => {
  ...
  describe("stratifiedSample", () => {
    it("should perform stratified sampling with proportional allocation", async () => {
      ...
      const mockSampleResponse1: OpenAlexResponse<{
        id: string;
      ...
      const mockSampleResponse2: OpenAlexResponse<{
        id: string;
        publication_year: number;
      }> = {
      ...
      const mockSampleResponse3: OpenAlexResponse<{
        id: string;
        publication_year: number;
      }> = {
      ...
      const result = await samplingApi.stratifiedSample(
        "works",
        "publication_year",
        {
          sample_size: 100,
    ...
    it("should fallback to regular sample when grouping not supported", async () => {
      ...
      const mockFallbackResponse: OpenAlexResponse<{
        id: string;
        display_name: string;
      }> = {
        results: [
    ...
    it("should handle sampling errors gracefully", async () => {
      ...
      const mockGroupResponse: OpenAlexResponse<{
        ...
        group_by: [
          {
            key: "error-stratum",
            key_display_name: "Error Stratum",
            count: 100,
          },
        ],
      };
  ...
  describe("temporallyDiverseSample", () => {
    it("should create temporally diverse sample across periods", async () => {
      const mockRecentResponse: OpenAlexResponse<{
        id: string;
        publication_year: number;
      }> = {
      ...
      const mockModernResponse: OpenAlexResponse<{
        id: string;
        publication_year: number;
      }> = {
      ...
      const mockEarlyResponse: OpenAlexResponse<{
        id: string;
        publication_year: number;
      }> = {
      ...
      const mockHistoricalResponse: OpenAlexResponse<{
        id: string;
        publication_year: number;
      }> = {
  ...
  describe("citationWeightedSample", () => {
    it("should create citation-weighted sample with 70/30 split", async () => {
      const mockHighCitedResponse: OpenAlexResponse<{
        id: string;
      ...
      const mockRegularResponse: OpenAlexResponse<{
        id: string;
        cited_by_count: number;
      }> = {
        results: [
  ...
  describe("abTestSample", () => {
    it("should create two independent sample groups and detect overlap", async () => {
      const mockGroupAResponse: OpenAlexResponse<{
        id: string;
        display_name: string;
      }> = {
        results: [
      ...
      const mockGroupBResponse: OpenAlexResponse<{
        id: string;
        display_name: string;
      }> = {
        results: [
  ...
  describe("qualitySample", () => {
    ...
    it("should apply default quality filters for unknown entity types", async () => {
      ...
      await samplingApi.qualitySample("concepts" as "works", {
        sample_size: 5,
      });
  ...
  describe("getQualityFilters", () => {
    it("should return correct quality filters for each entity type", () => {
      ...
      expect(samplingApiWithPrivateMethods.getQualityFilters("works")).toBe(
        "has_doi:true,is_oa:true",
      );
      expect(samplingApiWithPrivateMethods.getQualityFilters("authors")).toBe(
        "works_count:>5",
      );
      expect(samplingApiWithPrivateMethods.getQualityFilters("sources")).toBe(
        "works_count:>100",
      );
      expect(
        samplingApiWithPrivateMethods.getQualityFilters("institutions"),
      ).toBe("works_count:>50");
      expect(samplingApiWithPrivateMethods.getQualityFilters("unknown")).toBe(
        "works_count:>1",
      );
    });
  });
