...
describe("GroupingApi", () => {
  ...
  describe("groupBy", () => {
    it("should group entities by specified field with default parameters", async () => {
      const mockResponse: OpenAlexResponse<{
        group_by?: Array<{
          key: string;
    ...
    it("should apply min_count filtering and group_limit", async () => {
      ...
      const result = await groupingApi.groupBy(
        "works",
    ...
    it("should handle missing group_by in response", async () => {
      ...
      await expect(
        groupingApi.groupBy("authors", "unsupported_field"),
      ).rejects.toThrow(
  ...
  describe("getTemporalTrends", () => {
    it("should get temporal trends with default parameters", async () => {
      const mockMainGroupsResponse: OpenAlexResponse<{
        group_by?: Array<{
          key: string;
          key_display_name: string;
          count: number;
      ...
      const mockTemporalResponse: OpenAlexResponse<{
        group_by?: Array<{
          key: string;
          key_display_name: string;
          count: number;
      ...
      const mockOverallResponse: OpenAlexResponse<{
        group_by?: Array<{
          key: string;
          key_display_name: string;
          count: number;
      ...
      const result = await groupingApi.getTemporalTrends(
        "works",
        "type",
        "publication_year",
      );
    ...
    it("should handle custom year ranges and time fields", async () => {
      ...
      const mockTemporalResponse: OpenAlexResponse<{
        ...
        group_by: [{ key: "2015", key_display_name: "2015", count: 30 }],
      };
      ...
      const mockOverallResponse: OpenAlexResponse<{
        ...
        results: [],
        meta: { count: 100, db_response_time_ms: 50, page: 1, per_page: 25 },
        group_by: [{ key: "2015", key_display_name: "2015", count: 100 }],
      };
      ...
      await groupingApi.getTemporalTrends(
        "authors",
        "country",
        "created_date",
        params,
      ...
      expect(mockClient.getResponse).toHaveBeenNthCalledWith(2, "authors", {
        filter:
          "country:single-group,from_created_date:2015-01-01,to_created_date:2015-12-31",
    ...
    it("should handle errors in temporal breakdown gracefully", async () => {
      ...
      const mockOverallResponse: OpenAlexResponse<{
        ...
        results: [],
        meta: { count: 200, db_response_time_ms: 50, page: 1, per_page: 25 },
        group_by: [{ key: "2020", key_display_name: "2020", count: 200 }],
      };
  ...
  describe("multiDimensionalGroup", () => {
    it("should perform basic two-dimensional grouping", async () => {
      const mockPrimaryResponse: OpenAlexResponse<{
        group_by?: Array<{
          key: string;
          key_display_name: string;
          count: number;
        ...
        group_by: [
          {
            key: "journal-article",
            key_display_name: "Journal Article",
            count: 150,
          },
      ...
      const mockSecondaryResponse: OpenAlexResponse<{
        group_by?: Array<{
          key: string;
          key_display_name: string;
          count: number;
      ...
      mockClient.getResponse
        ...
        .mockResolvedValue({
          ...
          group_by: [
            { key: "combined", key_display_name: "Combined", count: 75 },
          ],
        });
  ...
  describe("getTopPerformersByGroup", () => {
    it("should get top performers by group with default parameters", async () => {
      const mockGroupResponse: OpenAlexResponse<{
        group_by?: Array<{
          key: string;
          key_display_name: string;
          count: number;
    ...
    it("should handle custom parameters and limits", async () => {
      ...
      await groupingApi.getTopPerformersByGroup(
        "works",
        "type",
        "works_count",
        {
  ...
  describe("getDistributionStats", () => {
    it("should calculate distribution statistics without percentiles", async () => {
      ...
      const result = await groupingApi.getDistributionStats(
        "authors",
        "impact_level",
      );
  ...
  describe("percentile", () => {
    it("should calculate percentiles correctly", async () => {
      ...
      expect(groupingApiWithPrivateMethods.percentile(sortedArray, 25)).toBe(
        3.25,
      );
      expect(groupingApiWithPrivateMethods.percentile(sortedArray, 50)).toBe(
        5.5,
      );
      expect(groupingApiWithPrivateMethods.percentile(sortedArray, 75)).toBe(
        7.75,
      );
      expect(groupingApiWithPrivateMethods.percentile(sortedArray, 90)).toBe(
        9.1,
      );
    });
