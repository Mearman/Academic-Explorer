...
describe("KeywordsApi", () => {
	...
	describe("getKeyword", () => {
		it("should fetch a single keyword by ID", async () => {
			...
			expect(mockClient.getById).toHaveBeenCalledWith("keywords", "K123456789", {});
		...
		it("should pass additional parameters to client", async () => {
			...
			expect(mockClient.getById).toHaveBeenCalledWith("keywords", "K123456789", params);
		});
	});
	...
	describe("getKeywordsStats", () => {
		it("should calculate keywords statistics", async () => {
			...
			const mockSampleResponse: OpenAlexResponse<Keyword> = {
				results: [
					{ id: "K1", display_name: "AI", works_count: 100, cited_by_count: 500 },
					{ id: "K2", display_name: "ML", works_count: 200, cited_by_count: 800 },
				] as Keyword[],
				meta: {
					count: 1000,
					db_response_time_ms: 50,
			...
			expect(mockClient.getResponse).toHaveBeenNthCalledWith(1, "keywords", { per_page: 1 });
			expect(mockClient.getResponse).toHaveBeenNthCalledWith(2, "keywords", { per_page: 1000 });
		...
		it("should include additional parameters in stats calculation", async () => {
			...
			expect(mockClient.getResponse).toHaveBeenNthCalledWith(1, "keywords", { ...params, per_page: 1 });
			expect(mockClient.getResponse).toHaveBeenNthCalledWith(2, "keywords", { ...params, per_page: 100 });
		});
	});
	...
	describe("getTrendingKeywords", () => {
		it("should get trending keywords for year range", async () => {
			...
			expect(mockClient.getResponse).toHaveBeenCalledWith("keywords", {
				filter: "from_created_date:2020-01-01,to_created_date:2023-12-31,works_count:>10",
				sort: "works_count",
			});
		});
