...
export class ConceptsApi {
	...
	private isQueryParams(params: unknown): params is QueryParams {
		...
		if ("sort" in params) {
			const sortValue = (params as Record<string, unknown>).sort;
	 ...
	 */
	private isStrictConceptsQueryParams(params: unknown): params is StrictConceptsQueryParams {
	...
	async getConcepts(params: StrictConceptsQueryParams | QueryParams = {}): Promise<OpenAlexResponse<Concept>> {
		...
		// Otherwise, convert from StrictConceptsQueryParams
		if (this.isStrictConceptsQueryParams(params)) {
			return this.client.getResponse<Concept>("concepts", toQueryParams(params));
		}
		// Default case - treat as basic params
	...
	async searchConcepts(
		...
		const params: StrictConceptsQueryParams = select !== undefined
			? { ...baseParams, select }
			: baseParams;
	 ...
	 * ```
	 */
	async getRandomConcepts(params: StrictConceptsQueryParams = {}): Promise<OpenAlexResponse<Concept>> {
		return this.getConcepts({
			...params,
			sort: "random",
		});
	...
	async getAllConcepts(
		...
		return this.client.getAll<Concept>("concepts", toQueryParams(params), maxResults);
	}
    ...
    levels_distribution: Record<number, number>;
		...
		const totalWorks = sample.results.reduce((sum, concept) => sum + concept.works_count, 0);
		const totalCitations = sample.results.reduce((sum, concept) => sum + concept.cited_by_count, 0);

		// Calculate level distribution
		const levelsDistribution: Record<number, number> = {};
		sample.results.forEach(concept => {
			levelsDistribution[concept.level] = (levelsDistribution[concept.level] || 0) + 1;
		});
	 ...
	 * ```
	 */
	async getHighlyCitedConcepts(params: StrictConceptsQueryParams = {}): Promise<OpenAlexResponse<Concept>> {
		const filters: ConceptsFilters = {
			"cited_by_count": ">1000",
		};
