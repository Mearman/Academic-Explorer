...
describe("AuthorsApi", () => {
  ...
  describe("getAuthor", () => {
    ...
    it("should normalize ORCID URL format", async () => {
      ...
      const result = await authorsApi.getAuthor(
  ...
  describe("ORCID validation", () => {
    describe("isValidOrcid", () => {
      ...
      it("should validate ORCID URL format", () => {
        expect(
          authorsApi.isValidOrcid("https://orcid.org/0000-0003-1613-5981"),
        ).toBe(true);
        expect(
          authorsApi.isValidOrcid("http://orcid.org/0000-0002-1825-0097"),
        ).toBe(true);
        expect(authorsApi.isValidOrcid("orcid.org/0000-0001-2345-678X")).toBe(
          true,
        );
      });
      ...
      it("should handle malformed inputs", () => {
        ...
        expect(authorsApi.isValidOrcid("https://orcid.org/invalid")).toBe(
          false,
        ); // Invalid ORCID in URL
      });
    });
  });
  ...
  describe("searchAuthors", () => {
    ...
    it("should handle array filters correctly", async () => {
      ...
      expect(mockClient.getResponse).toHaveBeenCalledWith("authors", {
        ...
        sort: "relevance_score:desc",
        filter:
          "last_known_institution.id:I27837315|I123456789,x_concepts.id:C41008148",
      });
    });
  ...
  describe("getAuthorWorks", () => {
    ...
    it("should combine author filter with additional filters", async () => {
      ...
      expect(mockClient.getResponse).toHaveBeenCalledWith("works", {
        filter:
          "authorships.author.id:A2208157607,publication_year:>2020,is_oa:true,cited_by_count:10,type:journal-article|conference-paper,primary_topic.id:T10555|T11234",
      });
    });
    ...
    it("should pass additional query parameters", async () => {
      ...
      await authorsApi.getAuthorWorks(
        "A123",
        {},
        {
  ...
  describe("getAuthorCollaborators", () => {
    it("should analyze collaborations and return collaborator stats", async () => {
            ...
            authorships: [
              {
                author: {
                  ...
                  works_count: 10,
                  cited_by_count: 100,
                  ids: { openalex: "A2208157607" },
                  affiliations: [],
                  counts_by_year: [],
                  works_api_url:
                    "https://api.openalex.org/works?filter=author.id:A2208157607",
                  updated_date: "2023-01-01",
                  created_date: "2023-01-01",
                  summary_stats: {
                    "2yr_mean_citedness": 1.0,
      ...
      const mockCollaborator1: Partial<Author> = {
        id: "A123",
      ...
      const mockCollaborator2: Partial<Author> = {
        id: "A456",
    ...
    it("should filter authors with ORCID identifiers", async () => {
      const mockResponse: OpenAlexResponse<Author> = {
        results: [
          {
            id: "A1",
            display_name: "Author with ORCID",
            orcid: "0000-0003-1613-5981",
            works_count: 20,
            cited_by_count: 200,
            ids: { openalex: "A1", orcid: "0000-0003-1613-5981" },
            affiliations: [],
            counts_by_year: [],
            works_api_url: "https://api.openalex.org/works?filter=author.id:A1",
            updated_date: "2023-01-01",
    ...
    it("should fetch most cited authors with default limit", async () => {
      const mockResponse: OpenAlexResponse<Author> = {
        results: [
          {
            id: "A1",
            display_name: "Highly Cited Author",
    ...
    it("should fetch most productive authors by works count", async () => {
      const mockResponse: OpenAlexResponse<Author> = {
        results: [
          {
            id: "A1",
            display_name: "Productive Author",
            works_count: 500,
            cited_by_count: 5000,
            ids: { openalex: "A1" },
            affiliations: [],
  ...
  describe("streamAuthors", () => {
    it("should delegate to client stream method", async () => {
      ...
      const mockBatch2 = [{ id: "A3", display_name: "Author 3" }];
  ...
  describe("error handling", () => {
    it("should propagate client errors", async () => {
      ...
      await expect(authorsApi.getAuthor("A123")).rejects.toThrow(
        "API request failed",
      );
    });
    ...
    it("should handle network errors in search", async () => {
      ...
      await expect(authorsApi.searchAuthors("test query")).rejects.toThrow(
        "Network error",
      );
    });
    ...
    it("should handle errors in collaboration analysis", async () => {
      ...
      await expect(authorsApi.getAuthorCollaborators("A123")).rejects.toThrow(
        "Works fetch failed",
      );
    });
  });
