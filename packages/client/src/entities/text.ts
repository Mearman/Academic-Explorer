...
import type { Concept, QueryParams, OpenAlexResponse } from "../types";
...
export class TextApi {
  private readonly TEXT_VALIDATION_ERROR = "Text must be a non-empty string";
  private readonly TEXT_CONCEPTS_ENDPOINT = "text/concepts";
  ...
  async extractConceptsFromTitle(
    ...
    if (!title || typeof title !== "string" || title.trim().length === 0) {
      throw new Error(this.TEXT_VALIDATION_ERROR);
    }
    ...
    const response = await this.client.getResponse<Concept>(
      this.TEXT_CONCEPTS_ENDPOINT,
      params,
    );
