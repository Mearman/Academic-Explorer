...
import { OpenAlexBaseClient, OpenAlexRateLimitError } from "./client";
import type { OpenAlexClientConfig } from "./client";
import { OpenAlexClient } from "./openalex-client";
import type { OpenAlexResponse, QueryParams } from "./types";
import { RATE_LIMIT_CONFIG, RETRY_CONFIG, calculateRetryDelay } from "./internal/rate-limit";
...
export class CachedOpenAlexClient extends OpenAlexBaseClient {
  ...
  constructor(config: CachedOpenAlexClientConfig = {}) {
    ...
    logger.debug("CachedOpenAlexClient initialized", {
  ...
  override async getResponse<T>(
    ...
    try {
      ...
      const plan = await this.cache.optimizeRequest(entityType, [], undefined, convertedParams);

      logger.debug("Generated optimization plan", {
        entityType,
      ...
      if (plan.requiredApiCalls.length > 0) {
        ...
        logger.debug("Complete cache hit - no API requests needed", { entityType });
      }
      ...
      logger.debug("Cached request completed", {
        entityType,
      ...
      logError("Synthetic cache request failed, falling back to direct API", error);
  ...
  override async getById<T>(
    ...
    try {
      ...
      if (hasAllFields && Object.keys(cachedData).length > 0) {
        ...
        this.requestMetrics.cacheHits++;
        logger.debug("Complete cache hit for single entity", {
          entityType,
          entityId: id,
      ...
      const missingFields = requestedFields.filter(field => !cachedFields.includes(field));
      ...
      if (missingFields.length > 0) {
        ...
        logger.debug("Made surgical request for missing fields", {
          entityType,
          entityId: id,
          missingFields,
      ...
      logError("Cached getById failed, falling back to direct API", error);
  ...
  async promoteEntity(entityType: EntityType, entityId: string, fields?: string[]): Promise<void> {
    ...
    try {
      ...
      logError("Failed to promote entity", error);
    }
  }
  ...
  async clearCache(): Promise<void> {
    ...
    try {
      ...
      logger.debug("Cleared all cache data and reset metrics");
    } catch (error) {
  ...
  setCacheEnabled(enabled: boolean): void {
    ...
    logger.debug(`Cache ${enabled ? "enabled" : "disabled"}`);
  }
  ...
  private async updateCachesWithResults(
    ...
    try {
      ...
      logError("Failed to update caches with results", error);
    }
  }
  ...
  private async getAvailableFields(entityType: EntityType, entityId: string): Promise<string[]> {
    try {
      return await this.cache.getEntityFields(entityType, entityId);
    } catch {
      return [];
    }
  }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (this.hasStatusCode(error) && error.statusCode === 429) {
          ...
          logger.debug(`Rate limited, waiting ${String(waitTime)}ms before retry (attempt ${String(attempt + 2)}/${String(maxRetries + 1)})`, {
          ...
          logger.debug(`Retrying operation (attempt ${String(attempt + 2)}/${String(maxRetries + 1)})`, {
            error: error instanceof Error ? error.message : "Unknown error",
            waitTime
          });
          await new Promise(resolve => setTimeout(resolve, waitTime));
  ...
  private async getEntityWithFallback(
    ...
    if (apiCall) {
      ...
      return this.withEnhancedRequestHandling(apiCall, options);
    }
  ...
  clearStaticCache(): void {
    // Static data provider doesn't have clearCache method in stub
  ...
  destroy(): void {
    ...
    logger.debug("CachedOpenAlexClient destroyed");
  }
}
...
export function updateOpenAlexEmail(email: string | undefined): void {
  const config: { userEmail?: string } = {};

  const validEmail = email && email.trim().length > 0 ? email.trim() : null;
  if (validEmail) {
    config.userEmail = validEmail;
  }

  cachedOpenAlex.updateConfig(config);
}