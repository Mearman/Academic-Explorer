 ...
 */
function parseOpenAlexQueryUrl(url: string): { entityType: string; queryParams: URLSearchParams } | null {
	try {
		const urlObj = new URL(url);

		// Check if it's an OpenAlex domain
		if (!urlObj.hostname.includes('openalex.org') && !urlObj.hostname.includes('api.openalex.org')) {
			return null;
		}
		...
		const validEntityTypes = ['works', 'authors', 'institutions', 'sources', 'publishers', 'funders', 'topics', 'concepts', 'keywords'];
...
function ApiOpenAlexRoute() {
	...
	console.log(`ApiOpenAlexRoute: Called with _splat=${_splat}, search=${JSON.stringify(search)}`);
	...
	useEffect(() => {
		const resolveApiUrl = () => {
			try {
				...
				console.log(`ApiOpenAlexRoute: fullUrl=${fullUrl}`);

				// First check if it's an OpenAlex query URL (with parameters)
				const queryUrlParse = parseOpenAlexQueryUrl(fullUrl);
				console.log(`ApiOpenAlexRoute: queryUrlParse=${JSON.stringify(queryUrlParse)}`);
				if (queryUrlParse && queryUrlParse.queryParams.toString()) {
					...
					console.log(`ApiOpenAlexRoute: Navigating to ${listRoute} with search ${JSON.stringify(searchObj)}`);
					void navigate({
						to: listRoute,
						search: searchObj,
						replace: true,
	...
	return (
		<div style={{
			...
			<div style={{ fontFamily: "monospace", backgroundColor: "#f5f5f5", padding: "10px", borderRadius: "4px" }}>
				api.openalex.org/{decodeURIComponent(_splat ?? "")}{Object.keys(search).length > 0 ? `?${new URLSearchParams(search as Record<string, string>).toString()}` : ''}
			</div>
