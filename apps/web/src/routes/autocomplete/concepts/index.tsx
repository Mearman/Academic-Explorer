...
const autocompleteConceptsSearchSchema = z.object({
...
export const Route = createFileRoute("/autocomplete/concepts/")({
  ...
  validateSearch: autocompleteConceptsSearchSchema,
});

function AutocompleteConceptsRoute() {
  ...
  const {
    ...
    queryKey: ["autocomplete", "concepts", query],
    queryFn: async () => {
      if (!query.trim()) return [];

      logger.debug("autocomplete", "Fetching concept suggestions", { query });

      const response = await cachedOpenAlex.client.concepts.autocomplete(query);

      logger.debug("autocomplete", "Concept suggestions received", {
              ...
              <Card key={result.id} withBorder padding="md" shadow="sm">
                <Stack gap="xs">
                  <Group justify="space-between" wrap="nowrap">
                    <Anchor
                      href={`#/concepts/${result.id.replace("https://openalex.org/", "")}`}
                      fw={500}
                      size="md"
                    >
                      {result.display_name}
