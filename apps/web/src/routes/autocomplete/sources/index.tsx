...
const autocompleteSourcesSearchSchema = z.object({
...
export const Route = createFileRoute("/autocomplete/sources/")({
  ...
  validateSearch: autocompleteSourcesSearchSchema,
});

function AutocompleteSourcesRoute() {
  ...
  const {
    ...
    queryKey: ["autocomplete", "sources", query],
    queryFn: async () => {
      if (!query.trim()) return [];

      logger.debug("autocomplete", "Fetching sources suggestions", { query });

      const response = await cachedOpenAlex.client.sources.autocomplete(query);

      logger.debug("autocomplete", "Sources suggestions received", {
              ...
              <Card key={result.id} withBorder padding="md" shadow="sm">
                <Stack gap="xs">
                  <Group justify="space-between" wrap="nowrap">
                    <Anchor
                      href={`#/sources/${result.id.replace("https://openalex.org/", "")}`}
                      fw={500}
                      size="md"
                    >
                      {result.display_name}
