...
function isEntityType(entityType: string): entityType is EntityType {
  return ["works", "authors", "sources", "institutions"].includes(
    entityType as EntityType,
  );
}
// Cache functions - implement as stubs for now
function getCachedOpenAlexEntities(
...
// EntityFactory namespace implementation
const EntityFactory = {
...
export class GraphDataService {
  ...
  constructor(queryClient: QueryClient) {
    logger.debug(
      "graph",
      "GraphDataService constructor called",
      {},
      "GraphDataService",
    );
    ...
    this.relationshipDetectionService =
      createRelationshipDetectionService(queryClient);
    this.cache = {
      nodes: new Map(),
      edges: new Map(),
  ...
  async loadEntityGraph(entityId: string): Promise<void> {
    ...
    try {
      ...
      logger.debug(
        "graph",
        "Starting relationship detection for initial graph nodes",
        {
          nodeCount: nodes.length,
        },
        "GraphDataService",
      ...
      try {
        ...
        if (detectedEdges.length > 0) {
          logger.debug(
            "graph",
            "Adding detected relationship edges to initial graph",
            {
        ...
        logError(
          logger,
          "Failed to detect relationships for initial nodes",
          error,
          "GraphDataService",
          "graph",
        );
  ...
  async loadEntityIntoGraph(entityId: string): Promise<void> {
    ...
    try {
      ...
      if (primaryNodeId) {
        ...
        this.relationshipDetectionService
          .detectRelationshipsForNode(primaryNodeId)
          .then((detectedEdges) => {
            // Add detected relationship edges to the graph
            if (detectedEdges.length > 0) {
              logger.debug(
                "graph",
                "Adding detected relationship edges for newly added node",
                {
          ...
          .catch((error: unknown) => {
            logError(
              logger,
              "Failed to detect relationships for initial graph nodes",
              error,
              "GraphDataService",
              "graph",
            );
  ...
  async loadEntityIntoRepository(entityId: string): Promise<void> {
    ...
    try {
      ...
      logError(
        logger,
        "Failed to load entity into repository",
        error,
        "GraphDataService",
        "repository",
      );
  ...
  loadAllCachedNodes(): void {
    ...
    try {
      ...
      logger.debug(
        "graph",
        "Loaded all cached entities into graph",
        {
      ...
      logError(
        logger,
        "Failed to load cached nodes into graph",
        error,
        "GraphDataService",
        "graph",
      );
  ...
  async hydrateNodeToFull(nodeId: string): Promise<void> {
    ...
    if (!node) {
      logger.warn(
        "graph",
        "Node not found, cannot hydrate to full",
        { nodeId },
        "GraphDataService",
      );
      return;
    ...
    try {
      ...
      logger.debug(
        "graph",
        "Hydrating node to full with selective field loading",
        {
          nodeId,
      ...
      if (EntityFactory.isSupported(node.entityType)) {
        ...
        const entityInstance = EntityFactory.create(
          node.entityType,
          cachedOpenAlex,
        );
        ...
        logger.debug(
          "graph",
          "Using selective field loading for metadata fields",
          {
            nodeId,
        ...
        logger.debug(
          "graph",
          "Node hydrated with selective field loading",
          {
            nodeId,
  ...
  async detectRelationshipsForNode(nodeId: string): Promise<void> {
    try {
      await this.relationshipDetectionService.detectRelationshipsForNode(
        nodeId,
      );
    } catch (error) {
      logError(
        logger,
        "Failed to detect relationships for node",
        error,
        "GraphDataService",
        "graph",
      );
  ...
  async detectRelationshipsForAllNodes(): Promise<void> {
    ...
    logger.debug(
      "graph",
      "Starting relationship detection for all nodes",
      {
    ...
    for (let i = 0; i < allNodes.length; i += batchSize) {
      ...
      const batchPromises = batch.map((node) =>
        this.relationshipDetectionService
          .detectRelationshipsForNode(node.id)
      ...
      if (allDetectedEdges.length > 0) {
        logger.debug(
          "graph",
          "Adding detected edges from batch processing",
          {
      ...
      logger.debug(
        "graph",
        "Relationship detection batch completed",
        {
          batchIndex: Math.floor(i / batchSize) + 1,
          processedCount,
    ...
    logger.debug(
      "graph",
      "Relationship detection completed for all nodes",
      {
  ...
  async hydrateAllMinimalNodes(): Promise<void> {
    ...
    logger.debug(
      "graph",
      "Starting batch hydration of minimal nodes",
      {
    ...
    for (let i = 0; i < minimalNodes.length; i++) {
      ...
      try {
        ...
        if (i < minimalNodes.length - 1) {
          await new Promise((resolve) =>
            setTimeout(resolve, DELAY_BETWEEN_NODES),
          );
        }
        ...
        if ((i + 1) % BATCH_SIZE === 0 && i < minimalNodes.length - 1) {
          logger.debug(
            "graph",
            `Batch of ${String(BATCH_SIZE)} nodes completed, taking longer break`,
            {
              processedSoFar: i + 1,
    ...
    logger.debug(
      "graph",
      "Finished hydrating all minimal nodes",
      {
  ...
  async hydrateAllMinimalNodesImmediate(): Promise<void> {
    ...
    if (minimalNodes.length === 0) {
      logger.debug(
        "graph",
        "No minimal nodes to hydrate immediately",
        {},
        "GraphDataService",
      );
      return;
    ...
    logger.debug(
      "graph",
      "Starting immediate parallel hydration of all minimal nodes",
      {
        minimalNodeCount: minimalNodes.length,
      },
      "GraphDataService",
    ...
    const hydratePromises = minimalNodes.map((node) =>
      this.hydrateNodeToFull(node.id).catch((error: unknown) => {
        logger.warn(
          "graph",
          "Failed to hydrate minimal node in parallel batch",
          {
            nodeId: node.id,
            entityType: node.entityType,
            label: node.label,
    ...
    const successfulHydrations = results.filter(
    ...
    logger.debug(
      "graph",
      "Completed immediate parallel hydration of all minimal nodes",
      {
        totalRequested: minimalNodes.length,
  ...
  async expandNode({
    ...
    if (!force && alreadyExpanded) {
      ...
      if (allNodeIds.length > 1) {
        logger.debug(
          "graph",
          "Running relationship detection for already-expanded node",
          {
            nodeId,
        ...
        try {
          ...
          if (detectedEdges.length > 0) {
            logger.debug(
              "graph",
              "Found new relationships for already-expanded node",
              {
                nodeId,
                detectedEdgeCount: detectedEdges.length,
            ...
            logger.debug(
              "graph",
              "No new relationships found for already-expanded node",
              { nodeId },
              "GraphDataService",
            );
          }
          ...
          logError(
            logger,
            "Failed to detect relationships for already-expanded node",
            error,
            "GraphDataService",
            "graph",
          );
    ...
    try {
      ...
      if (!EntityFactory.isSupported(node.entityType)) {
        logger.warn(
          "graph",
          `Expansion not implemented for entity entityType: ${node.entityType}`,
          {
            nodeId,
            entityType: node.entityType,
          },
      ...
      if (!isEntityType(node.entityType)) {
        logger.error(
          "graph",
          "Invalid entity type for expansion",
      ...
      logger.debug(
        "graph",
        "Retrieved expansion settings for node expansion",
        {
          nodeId,
          entityType: node.entityType,
          expansionTarget,
      ...
      logger.debug(
        "graph",
        "Adding nodes to store before relationship detection",
        {
      ...
      if (relatedData.nodes.length > 0) {
        ...
        logger.debug(
          "graph",
          "Starting relationship detection for expanded nodes",
          {
            expandedNodeId: nodeId,
        ...
        try {
          ...
          if (detectedEdges.length > 0) {
            const finalEdgesWithRelationships = [
              ...finalEdges,
              ...detectedEdges,
            ];
            logger.debug(
              "graph",
              "Adding detected relationship edges",
              {
                expandedNodeId: nodeId,
          ...
          logError(
            logger,
            "Failed to detect relationships for expanded nodes",
            error,
            "GraphDataService",
            "graph",
          );
        ...
        if (allNodeIds.length > 1) {
          ...
          try {
            ...
            if (forceDetectedEdges.length > 0) {
              logger.debug(
                "graph",
                "Adding force-detected relationship edges",
                {
                  expandedNodeId: nodeId,
            ...
            logError(
              logger,
              "Failed to detect relationships with force=true",
              error,
              "GraphDataService",
              "graph",
            );
    ...
    try {
      ...
      for (const entityType of entityTypes) {
        ...
        try {
          switch (entityType) {
            ...
            case "authors": {
              const authorsResponse =
                await cachedOpenAlex.client.authors.getAuthors({
                  search: query,
                  per_page: limit,
                });
            ...
            case "institutions": {
              const institutionsResponse =
                await cachedOpenAlex.client.institutions.searchInstitutions(
                  query,
                  {
                    per_page: limit,
                  },
            ...
            case "topics": {
              const topicsResponse =
                await cachedOpenAlex.client.topics.getMultiple({
                  search: query,
                  per_page: limit,
                });
          ...
          logger.warn("api", `Failed to search ${entityType}`, {
            query,
            error,
          });
        }
      ...
      for (const result of flatResults) {
        try {
          ...
          logger.warn(
            "graph",
            "Could not detect entity type for search result",
            { result, error },
          );
        }
      }
  ...
  async createMinimalNode({
    ...
    try {
      ...
      logError(
        logger,
        `Failed to create minimal node for ${entityId}`,
        error,
        "GraphDataService",
        "graph",
      );
  ...
  private async loadMinimalDataInBackground({
    ...
    try {
      ...
      if (minimalNode) {
        ...
        logger.debug(
          "graph",
          "Background minimal data loading completed",
          {
            entityId,
      ...
      logger.debug(
        "graph",
        "Background minimal data loading failed silently",
        {
          entityId,
          entityType,
   ...
   */
  async expandAllNodesOfType(
    entityType: EntityType,
    options: ExpansionOptions = {},
  ): Promise<void> {
    logger.debug(
      "graph",
      "expandAllNodesOfType called",
      { entityType, options },
      "GraphDataService",
    );
    const store = graphStore.getState();
    ...
    if (allVisibleNodes.length === 0) {
      logger.debug(
        "graph",
        "No visible nodes found to expand",
        { entityType },
        "GraphDataService",
      );
      return;
    ...
    logger.debug(
      "graph",
      `Expanding all visible nodes to find ${entityType} entities`,
      {
        entityType,
    ...
    try {
      ...
      for (const node of allVisibleNodes) {
        try {
          ...
          logger.debug(
            "graph",
            `Found ${relatedEntityIds.length.toString()} ${entityType} entities related to ${node.id}`,
            {
              nodeId: node.id,
          ...
          for (const entityId of relatedEntityIds.slice(
            0,
            ...
            try {
              ...
              logger.warn(
                "graph",
                `Failed to fetch ${entityType} entity`,
                {
                  entityId,
                  error:
                    error instanceof Error
      ...
      logger.debug(
        "graph",
        `Completed expanding all visible nodes for ${entityType} entities`,
        {
          entityType,
   ...
   */
  private transformWork(work: Work): {
    nodes: GraphNode[];
    edges: GraphEdge[];
  } {
    const nodes: GraphNode[] = [];
   ...
   */
  private transformAuthor(author: Author): {
    nodes: GraphNode[];
    edges: GraphEdge[];
  } {
    const nodes: GraphNode[] = [];
   ...
   */
  private transformSource(source: Source): {
    nodes: GraphNode[];
    edges: GraphEdge[];
  } {
    const nodes: GraphNode[] = [];
   ...
   */
  private transformInstitution(institution: InstitutionEntity): {
    nodes: GraphNode[];
    edges: GraphEdge[];
  } {
    const nodes: GraphNode[] = [];
   ...
   */
  private transformSearchResults(results: OpenAlexEntity[]): {
    nodes: GraphNode[];
    edges: GraphEdge[];
  } {
    const nodes: GraphNode[] = [];
