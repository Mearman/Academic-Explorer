...
import type { OpenAlexEntity } from "@academic-explorer/openalex-client";
...
describe("RequestDeduplicationService", () => {
	...
	describe("getEntity", () => {
		...
		it("should handle fetch errors and clean up ongoing requests", async () => {
			...
			await expect(service.getEntity("W123456789", mockFetcher)).rejects.toThrow("Fetch failed");
		...
		it("should handle missing request entry error", async () => {
			...
			const ongoingRequests = (service as any).ongoingRequests as Map<string, any>;

			// Set up a scenario where has() returns true but get() returns undefined
			ongoingRequests.set("W123456789", undefined);

			await expect(service.getEntity("W123456789", mockFetcher)).rejects.toThrow("Request entry not found for W123456789");
		});
	});
	...
	describe("isCacheEntryFresh", () => {
		it("should return true for all entities (simplified implementation)", () => {
			...
			const isCacheEntryFresh = (service as any).isCacheEntryFresh.bind(service);
	...
	describe("detectEntityType", () => {
		...
		testCases.forEach(({ id, expected }) => {
			it(`should detect entity type ${expected} for ID ${id}`, () => {
				...
				// Use reflection to test private method
				const detectEntityType = (service as any).detectEntityType.bind(service);
	...
	describe("refreshEntity", () => {
		it("should clear cache and ongoing request then fetch fresh data", async () => {
			...
			expect(service.getStats().ongoingRequests).toBe(1);
			expect(queryClient.getQueryData(["entity", "W123456789"])).toEqual(mockEntity);
			...
			expect(queryClient.getQueryData(["entity", "W123456789"])).toEqual(freshEntity);
		});
	...
	describe("edge cases and error handling", () => {
		...
		it("should handle fetcher throwing non-Error objects", async () => {
			...
			await expect(service.getEntity("W123456789", mockFetcher)).rejects.toEqual(nonErrorObject);
		});
		...
		it("should clean up ongoing request on both success and failure", async () => {
			...
			await expect(service.getEntity("W987654321", mockFetcher)).rejects.toThrow();
			expect(service.getStats().ongoingRequests).toBe(0);
		});
