...
describe('API Redirect Tests', () => {
  ...
  describe('API Interceptor Redirects', () => {
    ...
    it.each(['should redirect API variations to canonical /api/openalex/ format'])('API redirects work for sample URLs', async () => {
    ...
    it('should handle entity type patterns correctly', () => {
      ...
      for (const pattern of entityTypePatterns) {
        const match = pattern.match(/^\/api\/(works|authors|sources|institutions|topics|publishers|funders|keywords|concepts)/);
        expect(match).toBeTruthy();
        if (match) {
          const expectedRedirect = pattern.replace(/^\/api\//, '/api/openalex/');
          expect(expectedRedirect).toMatch(/^\/api\/openalex\/(works|authors|sources|institutions|topics|publishers|funders|keywords|concepts)/);
        }
      }
    });
  });
