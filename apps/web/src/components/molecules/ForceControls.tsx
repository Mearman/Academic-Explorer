...
import { IconRotateClockwise, IconInfoCircle } from "@tabler/icons-react";
import { useGraphStore } from "@/stores/graph-store";
import { AnimatedLayoutContext } from "@/components/graph/animated-layout-context";
import {
  useSetUseAnimatedLayout,
  useIsAnimating,
  useRequestRestart,
...
export const ForceControls: React.FC = () => {
  ...
  const [forceParams, setForceParams] = useState<ForceParameters>(() => {
    ...
    return {
      ...
      chargeStrength:
        current.chargeStrength ?? DEFAULT_FORCE_PARAMS.chargeStrength,
      centerStrength:
        current.centerStrength ?? DEFAULT_FORCE_PARAMS.centerStrength,
      collisionRadius:
        current.collisionRadius ?? DEFAULT_FORCE_PARAMS.collisionRadius,
      collisionStrength:
        current.collisionStrength ?? DEFAULT_FORCE_PARAMS.collisionStrength,
      velocityDecay:
        current.velocityDecay ?? DEFAULT_FORCE_PARAMS.velocityDecay,
  ...
  const handleParameterChangeImmediate = useCallback(
    ({ param, value }) => {
      ...
      if (isRunning && animationContext?.updateParameters) {
        ...
        logger.debug(
          "graph",
          "Updating force parameters during active simulation",
          {
            param,
            value,
        ...
        logger.debug(
          "graph",
          "Starting animation with updated force parameter",
          {
            param,
            value,
  ...
  const handleInputChange = useCallback(
    ({ param, value }) => {
      ...
      if (constrainedValue !== numValue) {
        logger.debug(
          "graph",
          `Constraining param from ${String(numValue)} to ${String(constrainedValue)}`,
          {
            param,
  ...
  const handleInputBlur = useCallback(
    (param: keyof ForceParameters) => {
      ...
      if (constrainedValue !== currentValue) {
        logger.debug(
          "graph",
          `Correcting param on blur from ${String(currentValue)} to ${String(constrainedValue)}`,
        );
        setForceParams((prev) => ({ ...prev, [param]: constrainedValue }));
  ...
  const handleReset = () => {
    ...
    // Update the current layout with default parameters
    {
      ...
      if (isRunning && animationContext?.updateParameters) {
        ...
        logger.debug(
          "graph",
          "Updating force parameters to defaults during active simulation",
          {
            isAnimating: isCurrentlyAnimating,
            isPaused: isCurrentlyPaused,
            isRunning,
        ...
        logger.debug(
          "graph",
          "Starting animation with default force parameters",
          {
            hasContext: !!animationContext,
            isWorkerReady: animationContext?.isWorkerReady ?? "unknown",
            isAnimating: isCurrentlyAnimating,
