...
import {
  ...
  selectNetworkStats,
} from "@/stores/network-activity-store";
...
const NetworkActivitySection: React.FC = () => {
  ...
  const filteredRequests = selectFilteredRequests(state);
  const networkStats = selectNetworkStats(state);
  ...
  const requestGroups = useMemo(() => {
    const groups = {
      pending: [] as NetworkRequest[],
      completed: [] as NetworkRequest[],
      failed: [] as NetworkRequest[],
      cached: [] as NetworkRequest[],
    };
    ...
    return (
      <Card key={request.id} padding="sm" withBorder>
          ...
          >
            ...
            {request.metadata && Object.keys(request.metadata).length > 0 && (
              <div>
                ...
                <Code block>{JSON.stringify(request.metadata, null, 2)}</Code>
              </div>
            )}
          </Stack>
        </Collapse>
        ...
        {request.status === "pending" && (
          <Progress value={100} size="xs" animated color="blue" mt="xs" />
        )}
      </Card>
    );
  };
        ...
        <Stack gap="xs">
          ...
          <Group justify="space-between">
            ...
            <div>
              ...
              <Text fw={500} size="sm">
                {networkStats.averageResponseTime > 0
                  ? formatDuration(networkStats.averageResponseTime)
                  : "-"}
              </Text>
            </div>
            <div>
              ...
              <Text fw={500} size="sm">
                {networkStats.totalDataTransferred > 0
                  ? formatSize(networkStats.totalDataTransferred)
                  : "-"}
              </Text>
            </div>
          </Group>
          ...
          <Stack gap="xs">
            ...
            <Group grow>
              ...
              <Select
                ...
                onChange={(value) => {
                  setTimeRange(value ? parseInt(value, 10) : 24);
                }}
                size="sm"
              />
            </Group>
