...
const entityTypeOptions = [
  ...
  {
    type: "institutions" satisfies EntityType,
    label: "Institutions",
    icon: IconBuilding,
  },
  { type: "topics" satisfies EntityType, label: "Topics", icon: IconTag },
  {
    type: "publishers" satisfies EntityType,
    label: "Publishers",
...
export const AllNodesSection: React.FC = () => {
  ...
  const { filteredNodes, nodesByType } = useMemo(() => {
    ...
    if (showOnlyVisible) {
      filtered = filtered.filter((node) => visibleEntityTypes[node.entityType]);
    }

    // Group by entity type
    const grouped = filtered.reduce<Partial<Record<EntityType, GraphNode[]>>>(
      (acc, node) => {
  ...
  const handleBatchActions = useCallback(
    (action: "pin" | "unpin" | "expand" | "remove" | "select") => {
