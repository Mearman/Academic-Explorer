...
export const useGraphStore = (() => {
  const hook = <T>(selector?: (state: GraphState & GraphActions) => T) => {
    const state = graphStoreResult.store.getState();
    return selector
      ? selector(state as GraphState & GraphActions)
      : (state as GraphState & GraphActions);
  };
  return hook;
})();
...
export const graphActions = graphStoreResult.actions;
