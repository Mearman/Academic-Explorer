...
const result = createTrackedStore<AppActivityState, AppActivityActions>({
  config: {
    name: "app-activity",
    initialState: {
      // State
  ...
  actionsFactory: ({ set, get }) => {
    ...
    const actions: AppActivityActions = {
      clearOldEvents: () => {
      ...
      addEvent: (event) => {
        ...
        performRecompute();
        if (Object.keys(get().events).length > get().maxHistorySize) {
          actions.clearOldEvents();
        }
        logger.debug(
          "ui",
