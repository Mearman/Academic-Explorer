  ...
  (set, get) => ({
    ...
    setPaused: (paused) => {
      set((state) => {
        ...
        logger.debug(
          "graph",
          paused ? "Animation paused" : "Animation resumed",
        );
      });
    },
    ...
    updateAnimatedPositions: (positions) => {
      set((state) => {
        ...
        state._cachedAnimatedPositionsArray = positions.map((pos) => ({
          ...pos,
        }));
    ...
    updateStaticPositions: (positions) => {
      set((state) => {
        ...
        // Update cached array for React 19 compatibility
        state._cachedStaticPositionsArray = positions.map((pos) => ({
          ...pos,
        }));
    ...
    setUseAnimatedLayout: (use) => {
      set((state) => {
        ...
        logger.debug(
          "graph",
          `Animated layout ${use ? "enabled" : "disabled"}`,
        );
      });
    },
