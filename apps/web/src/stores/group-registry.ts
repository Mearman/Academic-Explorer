 ...
 */
export const registerGroupDefinition = (
  groupDefinition: ToolGroupDefinition,
): void => {
...
const getNextOrderNumber = (): number => {
  const maxOrder = Math.max(
    ...Object.values(GROUP_DEFINITIONS).map((def) => def.order ?? 0),
  );
  return maxOrder + 1;
};
...
export const updateGroupDefinition = (
  ...
  const existingDefinition = GROUP_DEFINITIONS[groupId] as
    | ToolGroupDefinition
    | undefined;
  ...
  function isIconComponent(
    icon: unknown,
  ): icon is React.ComponentType<{ size?: number; className?: string }> {
  ...
  const updatedDefinition: ToolGroupDefinition = {
    ...
    icon: iconComponent,
    description:
      primarySection.tooltip ?? `Group containing ${primarySection.title}`,
 ...
 */
export const getGroupDefinition = (
  categoryId: string,
