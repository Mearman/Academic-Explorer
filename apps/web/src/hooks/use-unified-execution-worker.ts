...
import { ExecutionMode } from "@academic-explorer/graph";
import type {
...
import type { EntityType, GraphNode, GraphEdge } from "@academic-explorer/graph";
import type { ExpansionOptions } from "@academic-explorer/shared-utils";
import type { ExpansionSettings } from "@academic-explorer/graph";
...
export function useUnifiedExecutionWorker(options: UseUnifiedExecutionWorkerOptions = {}) {
  ...
  const taskSystem = useUnifiedTaskSystem({
    mode: executionMode,
    maxConcurrency,
  ...
  const [systemStats, setSystemStats] = useState<Awaited<ReturnType<typeof taskSystem.getStats>>>({
    queueLength: 0,
    activeTasks: 0,
    processing: false,
