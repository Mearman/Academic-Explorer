...
const mockAuthorNode: GraphNode = {
  id: "author-1",
  label: "Test Author",
  entityType: "authors",
  entityId: "A456",
...
describe("useGraphPersistence", () => {
  ...
  const mockGraphSnapshot: GraphSnapshot = {
    ...
    nodes: [mockWorkNode, mockAuthorNode],
    edges: [mockEdge],
  ...
  describe("loadSession", () => {
    ...
    it("should use fitView when viewport not available", () => {
      ...
      localStorageMock.setItem(
        "academic-explorer-sessions",
        JSON.stringify([sessionWithoutViewport]),
      );
  ...
  describe("deleteSession", () => {
    beforeEach(() => {
      const sessions = [
        mockSession,
        { ...mockSession, id: "session_2", name: "Session 2" },
      ];
      localStorageMock.setItem(
        "academic-explorer-sessions",
  ...
  describe("updateSession", () => {
    ...
    it("should update only specified fields", () => {
      ...
      expect(sessions[0].metadata.description).toBe(
        mockSession.metadata.description,
      );
    });
