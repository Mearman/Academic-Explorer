...
class CIMonitor {
   ...
   */
  private execGhCommand(command: string): any {
    try {
      ...
      if (error.status === 1 && error.stderr?.includes('no runs found')) {
        return [];
      }
   ...
   */
  async getWorkflowRuns(workflowName: string = 'nx-ci.yml', limit: number = 10): Promise<WorkflowRun[]> {
  ...
  async getJobLogs(runId: number, jobId: number): Promise<string> {
    try {
      const logs = execSync(`gh run view ${runId} --log --job=${jobId}`, {
        cwd: this.repoPath,
        encoding: 'utf8',
  ...
  async watchCI(workflowName: string = 'nx-ci.yml', intervalSeconds: number = 30): Promise<void> {
    ...
    const checkStatus = async () => {
      ...
      console.log(`${statusIcon} Run #${latestRun.run_number} | ${latestRun.head_branch} | ${shortSha} | ${latestRun.status}`);
