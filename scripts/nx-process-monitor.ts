...
class NxProcessMonitor {
  ...
  private findNxProcesses(): ProcessInfo[] {
    ...
    try {
      if (isWindows) {
        ...
        const output = execSync('wmic process get Name,ProcessId,ParentProcessId,CommandLine /format:csv', {
          encoding: 'utf8',
          timeout: 10000
        });
   ...
   * Get monitor status
   */
  public getStatus(): { isRunning: boolean; processCount: number; pidFile: string; logFile: string } {
    return {
...
function main(): void {
  ...
  switch (command) {
    ...
    case 'scan':
      ...
      const scanMonitor = new NxProcessMonitor({ enableAutoCleanup: false });
