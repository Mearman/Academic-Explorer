 ...
 */
export async function extractOpenAlexPaths(
	options: ExtractOptions = {}
): Promise<ExtractionResult> {
	const { searchDir = DEFAULT_SEARCH_DIR, includeFilePaths = false } = options
...
async function cli() {
	...
	const format = args.includes("--urls") ? "urls" : args.includes("--both") ? "both" : "paths"
