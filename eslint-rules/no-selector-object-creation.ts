...
const ruleMessages = {
  ...
  selectorSpread: 'Avoid spread operators in selectors that create new objects. Use direct property access or cached state.',
...
function isZustandSelector(node: any): boolean {
  ...
  if (node.type === 'CallExpression' &&
      ...
      (node.callee.name === 'useStore' ||
       node.callee.name.endsWith('Store')) && // matches useGraphStore, useLayoutStore, etc.
      node.arguments.length > 0) {

    const firstArg = node.arguments[0];
    return firstArg.type === 'ArrowFunctionExpression' ||
